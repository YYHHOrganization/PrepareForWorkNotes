...
> 在八叉树（Octree）中，如果场景物体的位置发生了改变，更新八叉树的过程通常包括以下几个步骤：
>
> 1. **确定物体的新位置**：
>    - 首先，计算物体的新位置和其在空间中占用的区域。这通常涉及到物体的边界框（Bounding Box）或包围盒（Bounding Volume）的更新。
> 2. **检查物体的当前节点**：
>    - 确定物体当前所在的八叉树节点。如果物体的新位置仍然在同一个节点内，则不需要进行任何更新。
> 3. **从当前节点移除物体**：
>    - 如果物体的新位置超出了当前节点的边界，首先需要从当前节点中移除该物体。这可能涉及到从该节点的物体列表中删除物体。
> 4. **找到新的节点**：
>    - 根据物体的新位置，确定它应该插入到哪个八叉树节点中。这可能需要从根节点开始，逐层向下查找，直到找到合适的子节点。
> 5. **插入物体到新节点**：
>    - 将物体插入到新的八叉树节点中。如果新节点已经存在物体，可能需要处理节点的分裂（split）情况，以确保每个节点的物体数量保持在合理范围内。
> 6. **更新节点的状态**：
>    - 在物体被移除和插入后，可能需要更新相关节点的状态，例如物体计数、边界框等。
