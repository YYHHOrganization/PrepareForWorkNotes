...
class Bounds3
{
  public:
    ...
    int maxExtent() const //返回最长的边是x,y,z中的哪一个
    {
        Vector3f d = Diagonal();
        if (d.x > d.y && d.x > d.z)
            return 0;
    ...
    double SurfaceArea() const //返回表面积,用于计算SAH
    {
        Vector3f d = Diagonal();
    ...
    Vector3f Offset(const Vector3f& p) const //返回一个点相对于bounding box的偏移
    {
        Vector3f o = p - pMin;
        if (pMax.x > pMin.x)
            o.x /= pMax.x - pMin.x;
