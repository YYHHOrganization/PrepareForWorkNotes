...
方法1，先看完链接中的解释，还没懂可以看：

>您好！这个解法的确非常巧妙，但也因此比较抽象，很多人第一次看都会觉得困惑。我们一步一步把它拆解开，重点关注您的问题：**每个`num`是如何影响最终结果的**。
>
>为了看懂这个解法，我们需要先理解它的两个核心思想：
>
>1.  **思想一：按位统计求余。**
>    如果一个数字出现了三次，那么它在二进制下的每一位（0或1）也出现了三次。所有出现了三次的数字，它们在二进制的每一位上贡献的'1'的个数，都将是3的倍数。
>    而那个只出现一次的数字，它的某一位如果是'1'，那么这一位上'1'的总个数就不会是3的倍数。
>    因此，我们的目标就是：**计算出每个二进制位上'1'的总个数，然后对3取余**。最后将这些余数（只可能是0或1）组合起来，就是那个唯一的数字。
>
>2.  **思想二：用位运算模拟“模3计数器”。**
>    直接为32个二进制位分别创建计数器需要额外的空间（一个32元素的数组），不满足题目“常数级空间”的要求。所以，大神们想出了一种方法，用两个变量（`ones` 和 `twos`）来同时为所有32位进行计数。
>
>    这里的 `ones` 和 `twos` 可以理解为两个“状态记录器”。对于数字的**任意一个二进制位**来说：
>    *   如果这个位在 `twos` 中是1，在 `ones` 中是0，代表到目前为止，这个二进制位上'1'的累计总数是 `3n + 2`。
>    *   如果这个位在 `twos` 中是0，在 `ones` 中是1，代表到目前为止，这个二进制位上'1'的累计总数是 `3n + 1`。
>    *   如果这个位在 `twos` 中是0，在 `ones` 中是0，代表到目前为止，这个二进制位上'1'的累计总数是 `3n`。
>
>    这个计数器的工作模式就像一个三进制的加法器，状态转换如下：
>    **`状态0 (00)` -> `状态1 (01)` -> `状态2 (10)` -> `状态0 (00)`**
>
>---
>
>### 关键：每个`num`如何影响`ones`和`twos`
>
>现在我们来详细追踪示例 `nums = [2, 2, 3, 2]`，看看每个数字是如何更新 `ones` 和 `twos` 的。
>（为了清晰，我们只看低4位二进制）
>
>*   **初始状态**: `ones = 0` (0000), `twos = 0` (0000)
>*   **输入数字**: `2` (0010), `3` (0011)
>
>---
>
>**第一轮：处理第一个 `num = 2` (0010)**
>
>*   **计算新 `ones`**: `ones = ones ^ num & ~twos`
>    *   `~twos` 是 `~0000` -> `1111`
>    *   `num & ~twos` 是 `0010 & 1111` -> `0010`
>    *   `ones ^ (num & ~twos)` 是 `0000 ^ 0010` -> `0010`
>    *   **更新后 `ones` = 2 (0010)**
>
>*   **计算新 `twos`**: `twos = twos ^ num & ~ones` (注意：这里的 `ones` 是上面刚算出来的新值)
>    *   `~ones` 是 `~0010` -> `1101`
>    *   `num & ~ones` 是 `0010 & 1101` -> `0000`
>    *   `twos ^ (num & ~ones)` 是 `0000 ^ 0000` -> `0000`
>    *   **更新后 `twos` = 0 (0000)**
>
>*   **结果**: `ones` = 2, `twos` = 0。
>    *   **解读**: 对于第1位（从右数第二位），我们看到了一个'1'，所以它的计数状态变为1 (`ones`的该位为1)。其他位没看到'1'，状态不变。
>
>---
>
>**第二轮：处理第二个 `num = 2` (0010)**
>
>*   **当前状态**: `ones` = 2 (0010), `twos` = 0 (0000)
>
>*   **计算新 `ones`**: `ones = ones ^ num & ~twos`
>    *   `~twos` 是 `~0000` -> `1111`
>    *   `num & ~twos` 是 `0010 & 1111` -> `0010`
>    *   `ones ^ (num & ~twos)` 是 `0010 ^ 0010` -> `0000`
>    *   **更新后 `ones` = 0 (0000)**
>
>*   **计算新 `twos`**: `twos = twos ^ num & ~ones` (使用新的`ones=0000`)
>    *   `~ones` 是 `~0000` -> `1111`
>    *   `num & ~ones` 是 `0010 & 1111` -> `0010`
>    *   `twos ^ (num & ~ones)` 是 `0000 ^ 0010` -> `0010`
>    *   **更新后 `twos` = 2 (0010)**
>
>*   **结果**: `ones` = 0, `twos` = 2。
>    *   **解读**: 对于第1位，我们又看到了一个'1'，累计数是2。所以它的计数状态从1变为2（`ones`的该位变0，`twos`的该位变1）。
>
>---
>
>**第三轮：处理 `num = 3` (0011)**
>
>*   **当前状态**: `ones` = 0 (0000), `twos` = 2 (0010)
>
>*   **计算新 `ones`**: `ones = ones ^ num & ~twos`
>    *   `~twos` 是 `~0010` -> `1101`
>    *   `num & ~twos` 是 `0011 & 1101` -> `0001`
>    *   `ones ^ (num & ~twos)` 是 `0000 ^ 0001` -> `0001`
>    *   **更新后 `ones` = 1 (0001)**
>
>*   **计算新 `twos`**: `twos = twos ^ num & ~ones` (使用新的`ones=0001`)
>    *   `~ones` 是 `~0001` -> `1110`
>    *   `num & ~ones` 是 `0011 & 1110` -> `0010`
>    *   `twos ^ (num & ~ones)` 是 `0010 ^ 0010` -> `0000`
>    *   **更新后 `twos` = 0 (0000)**
>
>*   **结果**: `ones` = 1, `twos` = 0。
>    *   **解读**: 
>        *   对于第0位（最右位）：`num=3`带来一个'1'，之前计数是0，现在变1。所以`ones`的第0位变1。
>        *   对于第1位：`num=3`带来一个'1'，之前计数是2，现在变3（也就是模3余0）。所以`twos`的第1位变0。
>
>---
>
>**第四轮：处理第三个 `num = 2` (0010)**
>
>*   **当前状态**: `ones` = 1 (0001), `twos` = 0 (0000)
>
>*   **计算新 `ones`**: `ones = ones ^ num & ~twos`
>    *   `~twos` 是 `~0000` -> `1111`
>    *   `num & ~twos` 是 `0010 & 1111` -> `0010`
>    *   `ones ^ (num & ~twos)` 是 `0001 ^ 0010` -> `0011`
>    *   **更新后 `ones` = 3 (0011)**
>
>*   **计算新 `twos`**: `twos = twos ^ num & ~ones` (使用新的`ones=0011`)
>    *   `~ones` 是 `~0011` -> `1100`
>    *   `num & ~ones` 是 `0010 & 1100` -> `0000`
>    *   `twos ^ (num & ~ones)` 是 `0000 ^ 0000` -> `0000`
>    *   **更新后 `twos` = 0 (0000)**
>
>*   **结果**: `ones` = 3, `twos` = 0。
...
>### 最终结果
>
>循环结束后，`ones` 的值是3。为什么返回 `ones`？
>
>因为在遍历结束后：
>*   所有出现三次的数字，它们在每一位上贡献的'1'的个数都是3的倍数，所以这些位最终的计数状态都回到了0（即 `ones` 和 `twos` 的相应位都为0）。
>*   只出现一次的数字，它为自己是'1'的那些位贡献了1个'1'。这些位的最终计数状态是1（即 `ones` 的相应位为1，`twos` 的相应位为0）。
>
>因此，循环结束后，`twos` 必然为0，而 `ones` 的值就等于那个只出现一次的数字。在我们的例子中，最终 `ones` 是 3，这正是正确答案。
>
>**总结一下：**
>
>**每个`num`就是那个触发“模3计数器”状态变化的“输入信号”。** 这两行代码 `ones = ...` 和 `twos = ...` 是一个精巧设计的逻辑门组合，它能在一次循环内，让 `ones` 和 `twos` 的所有32个位，根据 `num` 对应的32个位，正确地完成一次计数状态的更新。
