...
## 【38】指针与数组` int(*p)[10] int*p[10] int*f(int i) int(*f)(int i) `
...
IPv4 使用 **32 位（4 字节）**地址，因此整个地址空间有 4 294 967 296（232232）个地址，也就是近43亿个地址。不过，有一些地址是特殊用途而保留的，如局域网专用地址（约1800万个地址）和组播地址（约2700万个地址），这样一来可直接在广域网上使用、路由的公网 IP 地址数量就更加少了。 

引用某个 IPv4地址时，可使用 W.X.Y.Z 的点分十进制表示形式，如 192.168.1.10 等。

由前面介绍的数据转换内容可以知道，每个 8 位二进制所能表示的最大数就是 28−1=256−1=25528−1=256−1=255 （最小数为0），所以 IPv4地址转换成十进制数后，每段8位二进制组的取值范围是 0~255。因为 IP 地址在计算机是以二进制表示的，32位就相当于 4 字节，所以在 IPv4 协议数据报格式，无论是源 IP 字段，还是目的 IP 地址字段都占 4 字节。
...
void quickSort(vector<int>&vec, int l, int r)
{
    ...
    while(i<j) //注意 没有等号的 否则会超时，因为等于之后就不会再移动了
    {
