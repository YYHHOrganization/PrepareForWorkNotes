...
> 注：部分答案放的是比较精简的版本，更为详细的介绍在其他的笔记里应该有所整理。精简版本有助于回顾核心知识，确保答案能够快速说中重点。:question:表示Y掌握的还不够牢固。
...
### （1）为什么红黑树插入删除效率高？为什么红黑树插入删除后迭代器没有失效？:question:
...
- 关于迭代器失效问题，可以先看一下这篇：[【C++ STL】迭代器失效的几种情况总结 - fengMisaka - 博客园](https://www.cnblogs.com/linuxAndMcu/p/14621819.html)
  - 对于关联容器(如 map, set,multimap,multiset)，删除当前的 iterator，仅仅会使当前的 iterator 失效，只要在 erase 时，递增当前 iterator 即可。这是因为 map 之类的容器，使用了红黑树来实现，插入、删除一个结点不会对其他结点造成影响。以删除为例，erase 迭代器只是被删元素的迭代器失效，可以采用`erase(iter++)`的方式删除迭代器。**虽然删除了一个元素，整棵树也会调整以符合红黑树的规范，但是单个节点在内存中的地址没有变化，变化的是各节点之间的指向关系。**(:thinking: 当然被删除节点的内存会被释放)
...
### （7）能用shared_ptr初始化unique_ptr吗？反过来可以吗？
...
- **编译时多态（静态多态）**：在编译器即可确定调用哪个函数，包括运算符重载、函数重载和**模板**；
...
- **衍生问题1：构建一个只能在栈上创建的类；**
  - 把构造函数，拷贝构造函数设为私有，提供一个静态的成员函数 **return** StackOnly();

- **衍生问题2：构建一个只能在堆上创建的类；**
  - 把operator new 设为私有，
  - 把构造函数设为私有，提供一个静态的成员函数。**return** **new** HeapOnly;
...
但是需要注意的是，这个是包含被包含的情况的，我们要如何把包含的情况额外判断呢？即输出 包含， 相交，和不相交三种
...
结合以上条件，我们可以编写一个函数来判断两个AABB的关系：

```python
def check_aabb_relation(A, B):
    Ax1, Ay1, Ax2, Ay2 = A
    Bx1, By1, Bx2, By2 = B
    ...
    if Ax1 < Bx2 and Ax2 > Bx1 and Ay1 < By2 and Ay2 > By1:
        ...
        if Ax1 <= Bx1 and Ax2 >= Bx2 and Ay1 <= By1 and Ay2 >= By2:
            return "A 包含 B"
        elif Bx1 <= Ax1 and Bx2 >= Ax2 and By1 <= Ay1 and By2 >= Ay2:
            return "B 包含 A"
        else:
            return "相交"
    else:
...
A = (1, 1, 4, 4)
B = (2, 2, 3, 3)
print(check_aabb_relation(A, B))  # 输出: 相交

A = (1, 1, 4, 4)
B = (0, 0, 5, 5)
print(check_aabb_relation(A, B))  # 输出: A 包含 B

A = (1, 1, 4, 4)
B = (5, 5, 6, 6)
print(check_aabb_relation(A, B))  # 输出: 不相交
```

这个函数可以根据输入的两个AABB输出它们的关系：包含、相交或不相交。
...
void printRelation(Relation rel) {
    switch (rel) {
        case CONTAINS_A:
            std::cout << "B 包含 A" << std::endl;
            break;
        case CONTAINS_B:
            std::cout << "A 包含 B" << std::endl;
            break;
        case INTERSECT:
...
其实上面的判断重叠包含等关系问题也可以使用面积判断，//面积=其中一个 包含   ；//=0 不相交 但是如果这样复杂度就大了，算了
