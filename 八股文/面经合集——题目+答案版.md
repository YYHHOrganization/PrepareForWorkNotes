...
3. **联立方程求解**：
   ...
   - 利用 \( (x + y)^2 = x^2 + y^2 + 2xy \)，得：
     ...
     xy = \frac{a^2 - b}{2}
     \]
   - 解二次方程 \( t^2 - at + \frac{a^2 - b}{2} = 0 \)，根即为缺失数 \( x \) 和 \( y \)。
...
- 相对于简单的赋值而言，装箱和取消装箱过程需要进行**大量的计算**。 对值类型进行装箱时，必须分配并构造一个新对象。 
...
- UI框架，会有各种类型的Message，在Open函数的时候传入的对象会使用装箱操作，在打开的特定UI View中的OnOpen函数中进行拆箱操作（因为他自己知道具体的Message是什么类），从而实现解耦。（待确认，但基本没问题）
