...
:sun_behind_large_cloud: 暂时没看 后面需要看
...
## 【4】enum class 的好处/和enum相比较:question::moneybag:
...
https://c.biancheng.net/view/6826.html
...
https://blog.csdn.net/weixin_44988779/article/details/108913561
...
>   - 1）每个结点非红即黑；
>   - 2）根节点是黑的；
>   - 3）如果一个结点是红色的，那么它的子节点就是黑色的（也就是不能出现连续的红节点，但是可以连续黑节点）；
...
![image-20250724215416759](assets/image-20250724215416759.png)
...
- 关于迭代器失效问题，可以先看一下这篇：[【C++ STL】迭代器失效的几种情况总结 - fengMisaka - 博客园](https://www.cnblogs.com/linuxAndMcu/p/14621819.html) 或者看下面有提及
  ...
  - 顺序容器：因为 vetor、deque 使用了连续分配的内存，`erase`操作删除一个元素导致后面所有的元素都会向前移动一个位置，这些元素的地址发生了变化，所以当前位置到容器末尾元素的所有迭代器全部失效。
...
>关于内联（`inline`）的缺点以及构造函数/析构函数的潜在问题，以下是逐步解释：
>
>---
>
>### 一、内联的缺点
...
AI详细简单解释见：D:\PGPostgraduate\githubNotePrepareForWork\PrepareForWorkNotes\八股文\C++\知识点\enable_shared_from_this\enable_shared_from_this.md
...
## 【13】关于函数重载，为什么不能仅靠返回值不同判断 :cat: :dog: 关于const :moneybag:
...
>   - **顶层`const`与底层`const`**：
>     - **顶层`const`**：表示**对象本身是常量**（如`int* const`中的指针本身不可变）。(记 顶层就是const了)
...
（3）**模板与智能指针可以无缝结合**。标准库中的智能指针（如 `std::shared_ptr` 和 `std::unique_ptr`）本身就是模板类，能够管理模板类的实例（在使用`wrapper`实现模板类型擦除的例子中就有智能指针，参考附录文件夹中的C++新特性md文档）。举个例子：
...
> 在C++中，`delete` 和 `delete[]` 能够准确释放内存的核心机制如下：
>
> ---
>
> ### **1. 内存分配的簿记机制**
> 当使用 `new` 或 `new[]` 分配内存时，**内存分配器**会在用户请求的内存块**头部或尾部**记录**元数据**（如分配的内存大小、对象数量等）。  
> 例如：  
> ```text
> | 元数据（如分配大小） | 用户实际使用的内存块 |
> ```
> 用户获得的指针指向**用户内存块的起始地址**，而非元数据位置。
>
> ---
>
> ### **2. `delete` 的实现逻辑**
> - **步骤**：  
>   1. 根据用户传入的指针，**向前偏移**找到元数据，读取分配的内存大小。  
>   2. 调用对象的析构函数（若为类类型）。  
>   3. 将**整个内存块**（元数据+用户内存）返还给系统。  
>
> - **无需传递大小的原因**：  
>   元数据中已记录分配大小，`delete` 直接通过指针定位元数据即可。
>
> ---
>
> ### **3. `delete[]` 的额外处理**
> 当释放数组（`new[]` 分配的内存）时：  
> 1. **元数据中记录元素数量**（如 `int[10]` 会记录 `10`）。  
> 2. **遍历调用每个元素的析构函数**（若元素是类类型）。  
> 3. **释放全部内存**（元数据+用户内存）。  
>
> **关键区别**：  
> - `delete[]` 必须根据元数据中的元素数量调用多次析构函数。  
> - 若误用 `delete` 释放数组：  
>   - **未定义行为**：可能仅调用第一个元素的析构函数，且释放的内存大小错误（破坏堆结构）。  
>
> ---
>
> ### **4. 底层实现示例**
> 以 **MSVC** 编译器为例：  
> - **分配 `new int[10]`**：  
>   - 头部记录 `10`（元素数量）及内存块总大小。  
> - **释放 `delete[]`**：  
>   - 读取头部中的 `10`，确保正确释放内存并调用析构函数（若适用）。  
>
> ---
>
> ### **5. 总结**
> | **操作**   | **核心机制**                           | **关键区别**                 |
> | ---------- | -------------------------------------- | ---------------------------- |
> | `delete`   | 通过元数据读取分配大小，释放整个内存块 | 仅处理单个对象               |
> | `delete[]` | 读取元素数量，遍历析构并释放内存       | 必须用于数组，确保析构正确性 |
>
> **注意**：  
> - 必须严格匹配 `new`/`delete` 和 `new[]`/`delete[]`，否则导致未定义行为。  
> - 元数据的具体实现（如存储位置、格式）因编译器和内存分配器而异，但对用户透明。



## 【18】函数调用的过程具体是怎么样的？比如入栈顺序 :cat: :fireworks:

举例子：

D:\PGPostgraduate\githubNotePrepareForWork\PrepareForWorkNotes\八股文\C++\知识点\函数调用过程 入栈顺序 通俗例子.md

> 函数调用时的栈操作顺序如下：
>
> - 1.caller的参数压栈：按照从右到左的顺序依次压入参数（比如`func(a,b,c)`,此时先压c，再压b，再压a）；
> - 2.返回地址：执行`call`指令，将**下一条指令地址**（返回地址）压入栈；
...
### 【19.1】unordered_map 默认多大? :new:

VISUAL STUDIO是8

![image-20250325002935134](assets/image-20250325002935134.png)

gpt:

>在 C++ 中，`unordered_map` 是一个基于哈希表的容器，它的底层实现使用了桶（buckets）来存储元素。关于 `unordered_map` 的默认大小和为什么不默认使用质数，我们可以从以下几个方面进行解释：
>
>### 1. `unordered_map` 的默认大小
>
>在 C++ 的标准库中，`unordered_map` 的默认初始桶数通常是 8。这意味着在创建一个空的 `unordered_map` 时，它会分配 8 个桶来存储元素。这个默认值可能会因不同的编译器和标准库实现而有所不同，但通常是一个相对较小的值，以便在存储少量元素时节省内存。
>
>### 2. 为什么不默认使用质数
>
>使用质数作为哈希表的桶数是一个常见的做法，因为质数可以帮助减少哈希冲突，从而提高哈希表的性能。然而，C++ 标准库的设计者可能选择了一个较小的默认值（如 8），主要是出于以下考虑：
>
>- **内存效率**：在许多应用场景中，`unordered_map` 可能只存储少量元素。使用较小的桶数可以减少内存的开销。对于小规模的数据，使用质数可能并不会带来显著的性能提升。
>
>- **动态扩展**：`unordered_map` 会根据元素的数量动态调整桶的数量。当元素数量超过当前桶数的负载因子（通常是 1.0），它会自动扩展桶的数量。扩展时，通常会选择一个更大的质数作为新的桶数，以减少冲突。
>
>- **实现复杂性**：在实现上，使用质数可能会增加一些复杂性，尤其是在动态扩展时。选择一个简单的默认值可以使实现更为简洁。
>
>### 3. 自定义桶数
>
>如果你希望在创建 `unordered_map` 时指定初始桶数，可以使用构造函数来实现。例如：
>
>```cpp
>#include <unordered_map>
>
>int main() {
>    // 创建一个初始桶数为 31 的 unordered_map
>    std::unordered_map<int, int> myMap(31);
>    
>    // 其他操作...
>    
>    return 0;
...
>```
>
>在这个例子中，我们创建了一个初始桶数为 31 的 `unordered_map`，这样可以在一定程度上减少哈希冲突，尤其是在预期会存储较多元素的情况下。
>
>### 总结
>
>`unordered_map` 的默认大小通常是 8，而不是质数，主要是为了内存效率和实现的简洁性。使用质数作为桶数的做法在动态扩展时会被采用，以提高性能。你可以根据需要自定义初始桶数，以优化性能。
...
>
>
>在 C++ 中，`unordered_map` 的实现细节和默认行为涉及以下几个方面：
>
>---
>
>### 1. **`unordered_map` 的默认桶数量**
>- **实现定义**：C++ 标准没有规定 `unordered_map` 的默认初始桶数量，具体数值由标准库的实现决定。例如：
>  - **GCC (libstdc++)**：默认初始桶数为 **10**。
>  - **MSVC**：可能采用类似的小初始值。
>- **动态调整**：当元素数量超过负载因子（默认为 `1.0`）时，会自动触发 `rehash`，桶数量会按实现定义的策略增长（通常是翻倍或选择一个质数）。
>
>---
>
>### 2. **如何编写 `unordered_map`**
>- **基本声明**：
>  ```cpp
>  #include <unordered_map>
...
>  ```cpp
>  // 初始化时指定桶数量为 100
>  std::unordered_map<int, int> myMap(100);
>  ```
>- **自定义哈希函数和负载因子**：
>  ```cpp
>  struct MyHash {
...
>  };
>  std::unordered_map<KeyType, ValueType, MyHash> myMap(100);
>  ```
>
>---
>
>### 3. **为什么默认桶数量不一定是质数？**
>- **性能与平衡**：质数作为桶数量可以减少哈希冲突，但计算质数需要额外开销。现代标准库实现（如 GCC 的 `libstdc++`）采用以下策略：
>  1. **初始桶数**：通常选择一个较小的非质数（如 10）以减少初始化开销。
>  2. **动态调整**：在 `rehash` 时，自动选择最接近且大于当前桶数的质数（例如从预定义的质数表中选取）。
>  
>- **示例：GCC 的质数表**
>  ```cpp
>  // libstdc++ 内部维护的质数表（部分示例）
>  constexpr size_t prime_list[] = {
>    5, 11, 23, 47, 97, 199, 409, 823, 1741, 3469, /* ... */
>  };
>  ```
>  当需要扩容时，从表中选取下一个质数作为新桶数量。
>
>- **哈希函数优化**：现代哈希函数（如 `std::hash`）通常能均匀分布键值，即使桶数非质数，冲突概率也可能较低。
>
>---
>
>### 4. **验证默认桶数量（示例代码）**
...
>}
>```
>在 GCC 中输出可能是 **10**，验证其初始桶数为非质数。
>
>---
>
>### 总结
>- **默认桶数**：由实现决定，通常为非质数（如 10）以优化初始化速度。
>- **动态扩容**：在 `rehash` 时切换为质数，保证哈希性能。
>- **编写建议**：无需手动指定质数，依赖标准库的自动优化即可。
...
更多补充可以看 D:\PGPostgraduate\githubNotePrepareForWork\PrepareForWorkNotes\八股文\C++\C++ 虚表相关.md最后一节
...
3. **typeid 运算符**：
   ...
   - 对于非多态类型，`typeid` 在编译时直接返回对应的 `type_info` 对象。注意：**非多态类型在编译时可以直接知道typeid是什么，详情可以看下面那篇官方文档。**
...
更多可以阅读官方文档：[typeid operator - cppreference.com](https://en.cppreference.com/w/cpp/language/typeid)
...
3. 如果检测到递归层数过大，可以认为快排出了严重的划分不均问题。为了避免爆栈问题，或者快排再次划分不均导致复杂度退化为 $O(n^2)$，于是使用堆排序。（比如管风琴快排就很难受，不管取中间还是左右作为pivot 都是极端值）
...
细节方面在`附录/C++ 11及之后的版本特性总结.md`当中有所总结。这里只记录一些关键词和注意事项，在复习的时候可以检验自己能否照着大纲说出重点内容。

### （1）C++ 11

- `nullptr`
- 类型推导`auto`，`decltype`，拖尾返回类型（auto与decltype结合）
- 区间迭代
  - 基于范围的`for`循环
  - 初始化列表以及`std::initializer_list`
- 模板增强（选学）
- 构造函数：委托构造，(继承构造:可能是非标准的,不用太研究)
- 新增容器：`std::array`，`std::forward_list`，无序容器（`std::unordered_map/std::unordered_multimap`和 `std::unordered_set/std::unordered_multiset`）
- 元组`std::tuple`（选学）
- 正则表达式（选学）
- 多线程支持（选学）
- Lambda表达式
...
- （选学）std::shared_timed_mutex与std::shared_lock，`std::quoted`,`std::exchange`，`std::integer_sequence`
...
- `typename`扩展
- `auto`的优化
- 条件表达式中支持初始化语句；（震惊！）(:star:)
- `lambda`表达式捕获`*this`(:star:)
- 折叠表达式（选学）
- 结构化绑定(:star:)
- `std::optional`(:star:)
...
>    {
>        public: 
>        inline /*constexpr */ int operator()(int y) //如果前面不加mutable，则Lambda函数生成的匿名类的重载operator()函数后面会有const，此时无法修改类内变量的值，当然引用捕获是没问题的，不需要加mutable
>        {
>            x++;
>            return x + y;
>        }
...
Lambda表达式的更多细节，推荐可以去这里看：[cpp_new_features/C++11常用新特性（二）.md at main · 0voice/cpp_new_features](https://github.com/0voice/cpp_new_features/blob/main/C%2B%2B11常用新特性（二）.md)
...
## 【31】C++程序运行的生命周期相关，如main函数执行前和执行后分别做了什么？ :moneybag:
...
> 在 C++ 中，全局 `static` 变量（包括全局作用域和命名空间作用域的静态变量）的初始化时机遵循以下规则：
>
> ---
>
> ### 1. **初始化阶段**
> 全局 `static` 变量的初始化分为两个阶段：
> - **静态初始化（Static Initialization）**  
>   在程序启动时（`main` 函数执行前）完成：
>   - 对于 **基本类型**（如 `int`、`float` 等），若初始值是 **常量表达式**，直接写入内存。
>   - 对于 **类类型**，若构造函数是 `constexpr` 且参数为常量，也会在静态初始化阶段完成。
>   ```cpp
>   static int a = 42;          // 静态初始化（常量表达式）
>   static constexpr double b = 3.14; // 静态初始化（constexpr）
>   ```
>
> - **动态初始化（Dynamic Initialization）**  
>   在程序启动后、`main` 函数执行前完成：
>   - 若变量需要调用 **非平凡的构造函数**（如依赖运行时计算或外部资源）。
>   ```cpp
>   static std::string s = "Hello";  // 动态初始化（调用构造函数）
>   static int c = get_value();      // 动态初始化（函数调用）
>   ```
>
> ---
>
...
>   ```cpp
>   // file.cpp
>   static int x = 10;   // 先初始化 x
>   static int y = x + 5; // 后初始化 y（此时 x 已初始化）
>   ```
>
> - **不同编译单元之间**  
>   **顺序不确定**（可能引发[静态初始化顺序问题](https://en.cppreference.com/w/cpp/language/siof)）。
>   ```cpp
>   // file1.cpp
>   static int a = 42;
>                                                      
>   // file2.cpp
>   static int b = a;  // 危险！a 可能尚未初始化
>                                                         
>   ```
>
> ---
>
> ### 3. 特例：局部静态变量
> 局部 `static` 变量（函数内的静态变量）的初始化时机不同：
> ```cpp
> void func() {
>     static int count = 0; // 首次调用 func 时初始化
>     count++;
> }
> ```
>
...
>
> ### 4. 初始化的线程安全性
> - C++11 起，静态变量的初始化是 **线程安全** 的（由编译器插入互斥锁）。
> - 但全局 `static` 变量的初始化本身在 `main` 之前完成，因此不涉及多线程竞争。
>
> ---
>
> ### 5. 未显式初始化的全局 `static` 变量
> 会被 **零初始化**（Zero-Initialized）：
> ```cpp
> static int z; // z = 0（零初始化）
> static std::vector<int> v; // v 的成员指针初始化为 nullptr
> ```
>
> ---
>
...
> | 变量类型           | 初始化时机                           | 初始化顺序保证     |
> | ------------------ | ------------------------------------ | ------------------ |
> | 全局 `static` 变量 | `main` 执行前，静态/动态初始化       | 同一文件按声明顺序 |
> | 局部 `static` 变量 | 首次执行到声明处时初始化             | 无跨文件顺序保证   |
> | 类静态成员变量     | 同全局 `static` 变量（需在类外定义） | 同一文件按声明顺序 |
>
> ---
>
> ### 规避初始化问题的实践
> 1. **使用 Construct On First Use**  
>    用函数内的局部 `static` 替代全局变量：
>    ```cpp
...
> ---
>
> 通过理解这些规则，可以避免因全局 `static` 变量初始化顺序导致的未定义行为。
...
#### 面试：**右值引用的特性是什么？应用场景有哪些？**

1. **特性**：

   - 通过`&&`声明，只能绑定到右值（临时对象、字面量）。
   - 核心目的：支持移动语义（转移资源所有权，避免深拷贝）。
...
2. **应用场景**：
   ...
   - 完美转发：
     ...
     template<typename T>
     void Forward(T&& arg) {
         func(std::forward<T>(arg)); // 保持值类别（左值/右值）
     }
     ```
   ...
   - 容器优化：
     ...
     vec.push_back(std::move(str)); // 转移而非拷贝字符串
     ```

**总结**：右值引用通过避免不必要的拷贝大幅提升性能，是现代C++高效编程的核心机制之一。
...
## 【37】字符串与字符数组 C++——C风格&string

https://blog.csdn.net/ksws0292756/article/details/79432329

见附录 D:\PGPostgraduate\githubNotePrepareForWork\PrepareForWorkNotes\八股文\附录\字符串与字符数组 C++——C风格&string.md

### （1）C++中的字符串驻留问题

C++ 标准**不强制要求字符串驻留**（但允许编译器自行优化），因此严格来说字符串驻留**不是 C++ 的语言特性**，而是编译器的**可选优化行为**。

补充：
```c++
const char* s1 = "danhengMarch7thStelle";
const char* s2 = "danhengMarch7thStelle";
s1 == s2; //此时是比较s1指向的地址是否和s2指向的地址相同
```



## 【38】指针与数组` int(*p)[10] int*p[10] int*f(int i) int(*f)(int i) ` :moneybag:
...
1. **`int(*p)[10]`**  
   ...
   - **含义**：`p` 是一个**数组指针**，指向包含10个`int`元素的数组。  
   - **解释**：`(*p)` 表明 `p` 是指针，`[10]` 表示它指向一个大小为10的数组，数组元素为 `int`。  
   - **示例**：  
     ```c
     int arr[10];
     int (*p)[10] = &arr;  // p指向整个数组arr
     ```
   
2. **`int *p[10]`**  
   
   - **含义**：`p` 是一个**指针数组**，包含10个`int*`类型的指针。  
   - **解释**：`[]` 优先级高于 `*`，因此 `p` 是大小为10的数组，每个元素是 `int*`。  
   - **示例**：  
     ```c
     int a, b, c;
     int *p[10] = {&a, &b, &c};  // 数组存储int指针
     //写成 int* p[10]更清晰
     ```
   
3. **`int* f(int i)`**  
   - **含义**：`f` 是一个**函数**，接受 `int` 参数，返回 `int*`（指向`int`的指针）。  
   - **解释**：函数声明中，`f` 是函数名，返回类型为 `int*`。  
   - **示例**：  
     ```c
     int* f(int i) {
         static int arr[10];
         return &arr[i];  // 返回int指针
     }
     ```

4. **`int(*f)(int i)`**  
   - **含义**：`f` 是一个**函数指针**，指向接受 `int` 参数并返回 `int` 的函数。  
   - **解释**：`(*f)` 表明 `f` 是指针，指向的函数参数为 `int`，返回值为 `int`。  
   - **示例**：  
     ```c
     int func(int x) { return x * 2; }
     int (*f)(int) = func;  // f指向函数func
     ```

### 关键区别总结：
| 声明             | 类型           | 描述                                    |
| ---------------- | -------------- | --------------------------------------- |
| `int(*p)[10]`    | 数组指针       | 指向含10个`int`的数组。                 |
| `int *p[10]`     | 指针数组       | 数组含10个`int*`元素。                  |
| `int* f(int i)`  | 返回指针的函数 | 函数`f`返回`int*`，接受`int`参数。      |
| `int(*f)(int i)` | 函数指针       | 指针`f`指向返回`int`且接受`int`的函数。 |

### 记忆技巧：
- **括号优先级**：`()` 和 `[]` 的优先级高于 `*`。括号内的 `*` 表示指针，否则优先结合数组或函数。
- **类型结合方向**：从变量名向外结合，先看是数组、函数还是指针。



## 【39】`define` 和 `const` 区别 以及define

| 特性         | `#define`（宏定义）    | `const`（常量）          |
| ------------ | ---------------------- | ------------------------ |
| **类型检查** | 无（预处理替换）       | 有（编译时检查）         |
| **作用域**   | 全局（文件内有效）     | 可局部/全局              |
| **内存分配** | 不分配内存             | 分配内存                 |
| **调试**     | 不可调试（替换后消失） | 可调试                   |
| **示例**     | `#define PI 3.14`      | `const float PI = 3.14;` |

**总结**：

- `#define` 是预处理指令，直接替换文本，无类型安全。
- `const` 是编译期常量，有**类型检查**，更安全。



#### 用debug工具能否debug  define

**答案**：**不能**，因为 `#define` 是**预处理指令**，在编译前就会被替换，调试工具（如 GDB、LLDB）无法直接跟踪宏定义。
...
> 可以参考的链接：[C++ 并发编程指南（10）线程间通信_c++ 线程间通信-CSDN博客 ](https://blog.csdn.net/cloud323/article/details/136620168)（被锁了）
>
...
int main() {
    ...
    t1.join();
    t2.join();
    std::cout << "Final value: " << shared_data << std::endl; // 200000
    return 0;
}
```

>1. **自动锁管理（std::lock_guard）**：在`increment`函数中使用`std::lock_guard`，其**构造函数自动加锁**，**析构函数自动解锁**，避免手动操作遗漏解锁导致死锁[2](https://blog.csdn.net/2201_75755162/article/details/145012474)[15](https://cloud.tencent.com/developer/article/2245100?areaId=106005)。
...
![image-20250323150513922](assets/image-20250323150513922.png)
...
3. **汇编**：将汇编代码转换为机器码（目标文件`.o`或`.obj`）。
4. **链接**：将多个目标文件和库文件合并为可执行文件。



## 【15】文件想要读到CPU里运行，有哪些步骤？

> **步骤描述：**
>
> 1. **文件加载**：操作系统将可执行文件从磁盘读入内存。
> 2. **解析格式**：识别文件格式（如ELF、PE（ELF 对应于UNIX 下的文件，而PE 则是Windows 的可执行文件）），提取代码段、数据段和依赖库信息。
> 3. **内存分配**：分配虚拟内存空间，映射代码段、数据段到指定地址，设置页表权限（如代码段可执行）。
> 4. 重定位与链接：
>    - **静态重定位**：调整代码中的绝对地址偏移。
>    - **动态链接**：加载依赖库（如.so/.dll），解析外部符号引用。
> 5. **初始化执行环境**：设置堆栈指针、程序计数器（指向入口函数，如`main`），传递命令行参数和环境变量。
> 6. **权限检查**：确保内存区域符合安全策略（如NX位防止代码注入）。
> 7. **移交控制权**：CPU从入口点开始执行指令，程序运行。
>
> **关键点**：文件→内存解析→内存分配→地址重定位→动态链接→环境初始化→执行。

更具体的内容被分散在了操作系统篇的其他题目里，可以对应复习。
...
### **一、管程（Monitor）和管道（Pipe）完全不一样！**
---

### **二、管程（Monitor）—— 线程的“会议室规则”**
#### **1. 用途**
- **解决多线程打架问题**：比如多个线程（员工）要修改同一个数据（共享的Excel表格）时，如何避免混乱。

#### **2. 核心规则**
- **会议室一次只进一个人**（互斥锁）：保证同一时间只有一个线程能操作共享资源。
- **等待机制**：如果资源被占用，其他线程在门口排队等待。
- **通知机制**：用“条件变量”让线程之间可以互相通知（比如喊一声：“我用完了，下一个进来吧！”）。

#### **3. 举个栗子🌰**
假设你和同事要修改同一个文档：
1. **管程就像会议室规则**：谁先到谁进，进去后锁门（其他人在外等）。
2. **你改完了，开门喊一声**：“我好了，下一个进来吧！”
3. **下一个同事才能进去修改**。
...
### **三、管道（Pipe）—— 进程间的“传声筒”**
#### **1. 用途**
- **进程间通信**（IPC）：比如程序A要把数据传给程序B，就像两个人在用对讲机说话。

#### **2. 核心规则**
- **单向传输**：数据从一端写入，另一端读取（像一条单行道）。
- **数据是字节流**：传输的是原始字节（比如文字、文件内容）。
- **常用于父子进程**：比如在命令行中用 `|` 符号（如 `ls | grep txt`）。

#### **3. 举个栗子🌰**
假设程序A是厨房，程序B是服务员：
1. **厨房（A）把做好的菜**（数据）放进传菜窗口（管道）。
2. **服务员（B）从窗口另一端取菜**，送给客人。
...
| **特性**     | **管程（Monitor）**          | **管道（Pipe）**           |
| ------------ | ---------------------------- | -------------------------- |
| **用途**     | 解决多线程共享资源的同步问题 | 进程间传输数据             |
| **作用对象** | 线程                         | 进程                       |
| **数据形式** | 保护共享数据（如变量、对象） | 传输字节流（如文字、文件） |
| **方向性**   | 无方向（只管互斥访问）       | 单向（一端写，另一端读）   |
| **典型案例** | Java的`synchronized`方法     | Linux命令的 `|` 符号       |

---

### **五、总结**
1. **管程（Monitor）**：是多线程的“交通警察”，防止多个线程操作共享数据时打架。
   - 关键词：**线程同步、互斥锁、条件变量**。
   
2. **管道（Pipe）**：是进程间的“数据传输管道”，让不同程序可以互相发送数据。
   - 关键词：**进程间通信、字节流、单向传输**。

---

### **考考你！🤔**
假设你写了一个多线程程序，两个线程要同时修改同一个银行账户的余额：
1. 应该用**管程**还是**管道**？
2. 如果要把这个账户数据发送给另一个程序处理，应该用哪个？
...
在 C++ 中，虽然没有直接称为 “管程（Monitor）” 的语法结构，但可以通过 **互斥锁（Mutex）** 和 **条件变量（Condition Variable）** 手动实现管程的行为。

可以看算法题目篇中的“【25】C++实现两个线程交替打印数字 1-10”,就类似管程。



### 操作系统：管程

**题目4**：关于管程（Monitor）的描述，正确的是？
A. 管程中允许多个线程同时执行
B. 管程通过条件变量（Condition Variables）实现同步
C. 管程的互斥性由程序员手动保证
D. 管程是一种基于忙等待的同步机制
...
1. **互斥性**：同一时间只有一个线程能进入管程（无需程序员手动保证）。
2. **条件变量**：通过`wait()`和`signal()`操作实现线程的阻塞与唤醒。
   选项A错误（管程是互斥的），选项C错误（互斥由编译器/系统保证），选项D错误（管程不依赖忙等待）。



## 【17】 忙等待

**忙等待（Busy Waiting）** 是一种线程或进程在等待某个条件满足时，持续占用 CPU 资源不断检查条件的机制。它的核心特点是：**不会主动让出 CPU**，而是通过循环（如 `while`）反复检查条件是否满足，直到条件达成。

---

### **举个栗子🌰**
假设你等朋友回复消息，有两种方式：
1. **正常等待**：把手机放一边，先做其他事，等通知（类似线程阻塞）。
2. **忙等待**：一直盯着手机屏幕，每秒刷新 100 次（浪费精力/CPU）。
...
### **为什么忙等待不好？**
1. **CPU 资源浪费**：循环会占用大量 CPU 时间，导致其他任务无法执行。
2. **性能问题**：在高并发场景下，可能导致系统卡顿甚至过热。
3. **优先级反转风险**：若低优先级线程占着 CPU 忙等待，高优先级线程可能无法及时运行。
...
| **特性**     | **忙等待**                             | **非忙等待（阻塞等待）**           |
| ------------ | -------------------------------------- | ---------------------------------- |
| **CPU 占用** | 持续占用 CPU                           | 主动让出 CPU，进入休眠状态         |
| **实现方式** | 循环检查条件（如 `while`）             | 依赖系统调用（如条件变量、信号量） |
| **适用场景** | 极少数特殊情况（如**自旋锁**短暂等待） | 绝大多数需要等待的场景             |
...
1. **使用条件变量（Condition Variable）**  
   ...
   cv.wait(lock, [] { return flag; }); // 阻塞，不占用CPU
...
2. **使用信号量（Semaphore）或事件（Event）**  
   操作系统提供的同步机制，直接让线程休眠。

3. **使用 `sleep`（谨慎使用）**  
   虽然能减少 CPU 占用，但无法及时响应条件变化。
   ```cpp
   while (!flag) {
       std::this_thread::sleep_for(100ms); // 降低检查频率
   }
   ```
...
### **忙等待的少数合理场景**
1. **自旋锁（Spinlock）**  
   当线程预计等待时间非常短（如多核 CPU 竞争锁），忙等待可能比线程切换更高效。  
   （但需结合原子操作或硬件支持，避免普通循环）
2. **内核开发或极底层优化**  
   某些情况下需直接控制 CPU 行为。
...
### **总结**
- **避免忙等待**：在用户态编程中，99% 的场景应使用条件变量、信号量等机制。
- **核心原则**：在等待时让出 CPU，提升系统整体效率。
...
之前三个方案都是懒汉式的，天生就线程不安全，为了线程安全我们使用了锁，使用了锁么就难免会带来额外开销。**很多时候，直接用饿汉式就很好。**
...
public sealed class Singleton //使用 sealed 关键字修饰，表示这个类不能被继承。
{
    ...
    static Singleton()
    {
        //这是一个显式的静态构造函数。它的存在告诉 C# 编译器不要将类型标记为 beforefieldinit，这意味着在首次访问 instance 之前，静态构造函数不会被调用。虽然在这个例子中静态构造函数是空的，但它的存在是为了确保 instance 的初始化在类加载时完成。C#类的静态构造函数可以参考：https://www.cnblogs.com/michaelxu/archive/2007/03/29/693401.html
    }
    private Singleton()
    {
...
- **策略模式：**定义一系列算法，使其可以互相替换，且算法的变化独立于客户端。
  ...
  - 实现方法：通过接口定义策略，运行时动态切换具体实现，适用于需要频繁调整或扩展算法的场景。可以参考[Unity游戏开发——策略模式 - 知乎](https://zhuanlan.zhihu.com/p/86951119)
    - 20250327补充：感觉其实可以将策略用委托或Action来保存，应该也是类似的设计模式。
...
（https://mp.weixin.qq.com/s?__biz=MzkxMDc1MDg1Nw==&mid=2247508504&idx=1&sn=b21196cfd1c3cbde80119240c9d2cd81&source=41&poc_token=HJuXhWij4UZReB09sektAEYe867DhQ8DlB5_53yV）

（IP层即网路层，IP层是「不可靠」的，它不保证网络包的交付、不保证网络包的按序交付、也不保证网络包中的数据的。
...
>
> **状态同步顾名思义就是同步各个客户端的状态，保证每一次操作后的状态是一致的。**通过开发服务端程序，把用户的操作作为输入实时上传到服务端，服务端通过计算返回结果给各个客户端，这样的过程就是状态同步。比如说玩家A开了一枪，那么A会给服务器发送自己开枪的报文，服务器**确认后**会转发给所有其他的客户端，这样其他客户端中的玩家A（叫做Replicated Player）就会同步开枪的状态，从而开枪。同时，如果这时服务器计算出了玩家A这一枪打中的玩家B，那么这个事件也会被同步给所有客户端。**游戏状态的计算是在服务器端的。**
>
> **状态同步的一个优势在于，可以只同步客户端的ROI（Region of Interest）区域。**这样的话针对远处区域（ROI范围之外）的外挂会少一些（不好做），但依旧不太好解决近处透视挂这种外挂。
>
> ### 1. 基本原理
>
> - **状态同步的战斗逻辑在服务端**
...
###  【4.1】**[帧同步](https://zhida.zhihu.com/search?content_id=107417849&content_type=Article&match_order=1&q=帧同步&zhida_source=entity)以及[状态同步](https://zhida.zhihu.com/search?content_id=107417849&content_type=Article&match_order=1&q=状态同步&zhida_source=entity)的区别，为什么我们项目采用了帧同步？ **:new:

- - 帧同步的逻辑全部在客户端，服务器只管进行数据转发即可；状态同步的逻辑放在服务器，客户端只管表现。由于帧同步的逻辑是放在客户端的，因而帧同步中的每个客户端都需要知道场景中的所有信息，才能正确的进行计算。而状态同步的逻辑在服务器，每个客户端只需要知道自己需要的信息进行表现即可。因此状态同步更适合[MMORPG](https://zhida.zhihu.com/search?content_id=107417849&content_type=Article&match_order=1&q=MMORPG&zhida_source=entity)这样的同步客户端数量较多，信息较为复杂的情况；而帧同步更适合[MOBA](https://zhida.zhihu.com/search?content_id=107417849&content_type=Article&match_order=1&q=MOBA&zhida_source=entity)这样的，**客户端数量固定**，更关注**实时性**的情况。
  - 我们项目定位是[即时格斗](https://zhida.zhihu.com/search?content_id=107417849&content_type=Article&match_order=1&q=即时格斗&zhida_source=entity)，对打击感、实时性要求比较高。并且我们的战斗场景仅作为一个表现存在，交互性相对比较少，而且参与战斗的客户端数量只有两个，战斗数据相对较少但是对实时性要求比较高，是一个典型的适用于帧同步的应用场景。如果采用状态同步，逻辑都在服务器计算，在同一帧中客户端的表现不一致，战斗的打击感会非常难调。没有打击感的格斗游戏已经失去了灵魂，可玩性大大降低，得不偿失。
  - 随着项目的推近，必定会推出新角色，增加新的战斗机制或表现。帧同步的逻辑都在客户端，因而只需要客户端进行制作即可，不需要服务器的参与。不像状态同步需要服务器修改逻辑客户端修改表现，保证了**服务器的稳定**且**节约人力以及沟通成本**，逻辑清晰，制作简单，**开发效率比较高**。
  
  
  
  https://zhuanlan.zhihu.com/p/86798204

### 【4.2】 **帧同步中产生不同步的原因以及解决方案?:new:**
...
- 数值的随机性。格斗类游戏中的事件随机性是它的一大魅力所在。例如闪避机制，暴击伤害等。由于帧同步中的逻辑都在客户端进行，每个客户端对这类事件进行计算，就没有唯一性。举例玩家A的暴击几率是80%，假设在第200帧中，玩家A进行了一次平A攻击，客户端A计算得到这次攻击产生了暴击伤害300，而客户端B计算得到的结果是暴击伤害280。这就导致了伤害不一致，随着战斗的进行两边客户端的差距会越来越大，得到不同的战斗结果。而帧同步就需要解决这类事件，保证两边随机结果的一致性。解决方案是客户端做[伪随机算法](https://zhida.zhihu.com/search?content_id=107417849&content_type=Article&match_order=1&q=伪随机算法&zhida_source=entity)。战斗开始时，服务器给客户端下发一个随机种子，通过自定义随机算法，保证每次的随机结果都是可控且一致的。
- 逻辑执行顺序不一致。我们项目中用到了很多第三方的插件（例如[PlayMaker](https://zhida.zhihu.com/search?content_id=107417849&content_type=Article&match_order=1&q=PlayMaker&zhida_source=entity)、[NGUI](https://zhida.zhihu.com/search?content_id=107417849&content_type=Article&match_order=1&q=NGUI&zhida_source=entity)）,这些插件的Update是没法由帧同步去控制的，各个客户端可能执行某个计算片段的时间并不是在同一逻辑帧内导致出现不同步的情况。unity的物理系统，动画系统也是如此。解决方案就是不使用第三方插件，使用成熟稳定的开源插件或者自己实现，**保证每个update都是自主可控的**。避免出现不可控导致的不同步的情况出现。
- 数学计算中的精度丢失。精度丢失的情况主要是出现在浮点运算中。不同的硬件环境下浮点数的运算结果可能是不一致的。可能在一帧中的差异性很小，但随着时间推移造成的雪球效应会导致客户端得到不同的结果。要保证在不同的硬件环境下，运算结果的一致性就需要使用[定点数](https://zhida.zhihu.com/search?content_id=107417849&content_type=Article&match_order=1&q=定点数&zhida_source=entity)替代浮点数，根据需求定一个比值参数（我们项目采用的是万分比）进行定点数和浮点数的转换。这样就保证了不会出现计算中的精度丢失问题。
- 接收网络数据不一致。我们项目在demo阶段时的帧同步采用的tcp的方案，在网络稍微差一点的情况下就出现战斗不流畅导致PVP战斗体验很差，后面我们采用UDP去自行实现可靠的网络收发。因为UDP的不可靠性以及复杂的网络环境，在出现网络波动的情况下，接收端接收到的消息是无序的，客户端在接收服务器发送的消息时，后发的消息可能会比先发的消息还要先接收到，如果这时候客户端不进行处理的话，就会出现客户端先处理后一条消息，再处理前一条消息的情况，这样就会造成客户端对于操作输入的顺序不一致，也是会造成最终的不同步的。解决方案就是对每一个发出的消息做一个自增的编号，根据编号的连续性确定消息的顺序，就算先收到后面的消息，也可以等待前面的消息收到之后进行顺序传入游戏逻辑中。

### 【4.3】王者荣耀为什么要在技术架构与网络同步方案上做出这些改变？:new:

https://www.easemob.com/news/868

 m  比如对于MOBA来说

帧同步本身对网络要求更加严苛，下发的执行序列是不允许丢包的，需要严格保证顺序性，包是12345，就必须是12345，如果丢包，必须要等到丢的包到达之后才能顺序后续执行。
...
1. **逻辑层**：核心战斗逻辑（技能伤害计算、移动同步）**使用定点数**，确保10ms级同步精度
2. **渲染层**：粒子特效、UI动画等**保留浮点数**，允许客户端差异化渲染

>在帧同步游戏中解决浮点数不同步问题和定点数的应用，可以从以下几个层面进行技术解析：
>
>---
>
>### **一、浮点数不同步的根源**
>1. **硬件差异**：不同CPU架构（x86/ARM）对浮点运算的中间精度处理不同（如x87 FPU使用80位中间精度，SSE使用64位）
>2. **编译器优化**：不同编译器的浮点运算优化策略可能导致舍入误差（如`-ffast-math`）
>3. **运算顺序**：如 `(a + b) + c ≠ a + (b + c)`，分布式运算顺序不一致导致误差累积
>
>---
>
>### **二、解决方案：定点数的核心技术细节**
>#### **1. 定点数的实现原理**
>- **存储格式**：将浮点映射为整数，例如 `Q16.16`（32位中高16位为整数，低16位为小数）
>- **运算规则**（以加法为例）：
>  ```cpp
>  // Q16.16定点数加法（自动处理溢出）
>  inline int32_t FixedAdd(int32_t a, int32_t b) {
>    int64_t tmp = (int64_t)a + (int64_t)b;
>    return (int32_t)std::clamp(tmp, INT32_MIN, INT32_MAX);
>  }
>  ```
>- **乘法优化**：通过移位避免64位中间结果
>  ```cpp
>  // Q16.16乘法（牺牲1位精度换取性能）
>  inline int32_t FixedMul(int32_t a, int32_t b) {
>    return (int32_t)(((int64_t)a * b) >> 16);
>  }
>  ```
>
>#### **2. 定点数的典型应用场景**
>- **物理引擎核心参数**：角色坐标、速度、碰撞体半径（如《王者荣耀》英雄的移动路径计算）
>- **确定性随机数**：技能暴击率、伤害波动（例如 `FixedRand(seed)` 保证全平台一致）
>- **时间同步**：将游戏时间戳转换为定点数（如 `FixedTime = (int32_t)(time * 65536)`）
>
>#### **3. 性能取舍**
>- **精度与范围的权衡**：`Q24.8` 适合大范围场景（如开放世界），`Q16.16` 适合高精度需求
>- **SIMD加速**：ARM NEON指令集可优化定点数批量运算（如移动端MOBA游戏的群体技能计算）
>
>---
>
>### **三、替代方案：浮点数的确定性处理**
>#### **1. IEEE-754严格模式**
>- 强制使用单精度浮点数（`float`而非`double`）
>- 禁用编译器浮点优化（GCC: `-fno-unsafe-math-optimizations`）
>- **案例**：《英雄联盟》早期版本通过统一SSE指令集实现浮点一致性
>
>#### **2. 软浮点库**
>- 自定义数学库替代标准库（如 `DeterministicMath.sin()`）
>- **开源方案**：Google的 `math-emu` 库提供跨平台确定性计算
>
>---
>
>### **四、王者荣耀的工程实践**
>1. **逻辑层**：核心战斗逻辑（技能伤害计算、移动同步）**使用定点数**，确保10ms级同步精度
>2. **渲染层**：粒子特效、UI动画等**保留浮点数**，允许客户端差异化渲染
>3. **混合策略**：场景加载时动态切换精度（如观战模式使用浮点数降精度节省带宽）
>
>---
>
>### **五、面试回答建议**
>```markdown
>1. **问题复现**：  
>   “在开发派对游戏时，我们遇到角色位置在不同客户端漂移的问题。经排查发现是浮点运算在移动端（ARM）和PC（x86）上的中间精度差异导致。”
>
>2. **解决方案**：  
>   “对坐标、速度等关键逻辑采用Q16.16定点数，通过自定义的`FixedMath`库统一运算。例如角色移动计算改为：  
>   `fixed_x = FixedAdd(fixed_x, FixedMul(speed, delta_time))`”
>
>3. **延伸思考**：  
>   “《王者荣耀》这类顶级MOBA会在逻辑帧严格使用定点数，但渲染层可能采用浮点数+插值平滑。这种分层设计兼顾了同步性和表现力。”
>```
>
>通过具体的技术细节和业务场景结合，既能体现工程深度，又展现了架构设计能力。
...
![img](assets/912059-20220102155117709-364882712.png)
...
## 【5】状态同步更细致的问题，网络同步有哪些优化技术？

> 这个问题是接在上一个问题之后的，在上一个问题中对帧同步和状态同步有了一个基本的认知，但有一个考察点在于对状态同步更细致一些的考察（关于如何做优化），因此整理在这里。

### （1）一些名词解释

> ### **1. Dumb Client Problem**  
> **定义**：客户端仅负责发送输入和渲染，所有逻辑由服务器计算，导致操作延迟高（等待服务器响应），体验不流畅。  
>
> ---
>
> ### **2. 解决方案：Client-Side Prediction & Server Reconciliation**  
> - **Client-Side Prediction**：客户端立即本地模拟操作结果（如移动），无需等待服务器响应，掩盖网络延迟。  
> - **Server Reconciliation**：  
>   - **流程**：  
>     1. 客户端发送输入至服务器，并存储在本地**环形缓冲区（Ring Buffer）**。  
>     2. 服务器处理输入后返回权威状态，客户端对比本地预测结果，若不一致则用服务器数据**修正**（如位置回退）。  
>   - **网络劣势**：网速差的玩家因修正频繁（如位置反复回退），体验更差。  
>
> ---
>
> ### **3. Packet Loss 服务器解决方案**  
> - **环形缓冲区复制操作**：服务器检测丢包时，自动用最后一次有效输入填充缺失帧（如连续3帧丢包，重复最后一帧输入），避免角色卡顿（以《原神》为例，联机的时候有时会观察到队友一直在往前走，实际上在他的客户端很可能已经停下了，但由于丢包服务器做了有效输入填充缺失帧的操作）。  
> - **代价**：操作延迟上升，但保证角色行为连续（牺牲时效性换流畅性）。  
>



### ==（2）Character Movement Replication==

> 在游戏网络同步的Character Movement Replication中,往往要进行内插值和外插值(外插值还有一个算法是Dead Reckoning,也详细解释一下),你能详细具体的解释一下内插和外插的原理和目的么?以及两者有什么问题,对比(可以用表格的形式给出),
...
```python
# 伪代码，展示RPC用法,
result = rpc_client.call("PlayerService.level_up", player_id=1001, exp=500) #RPC也可以异步+回调函数
# 实际执行流程：
1.客户端序列化数据(player_id, exp)->二进制数据
2.通过TCP/UDP发送到游戏服务器
3.服务器反序列化数据->执行真实的level_up逻辑
4.将结果(新等级)序列化后返回客户端
5.客户端反序列化结果->更新本地界面
```
...
主要参考文章：[UE4 浅谈网络同步中的RPC - 知乎](https://zhuanlan.zhihu.com/p/678112760)

> UE中的RPC类型分为以下三类，主要用于客户端与服务器的远程交互：
>
> | **类型**      | **调用位置** | **执行位置**           | **可靠性**      | **用途**                               |
> | ------------- | ------------ | ---------------------- | --------------- | -------------------------------------- |
> | **Server**    | 客户端       | 服务器                 | 可靠/不可靠可选 | 客户端请求服务器执行关键逻辑（如攻击） |
> | **Client**    | 服务器       | 指定客户端或所有客户端 | 可靠/不可靠可选 | 服务器向客户端推送状态更新或指令       |
> | **Multicast** | 服务器       | 服务器及所有客户端     | 默认不可靠      | 同步全局事件（如爆炸效果）             |
>
> ### 关键说明：
> 1. **与UE的关系**：UE通过`UFUNCTION`宏标记RPC类型（`Server`、`Client`、`NetMulticast`），实现多玩家同步。
> 2. **通用RPC类型**：RPC本身可分类为同步/异步、可靠/不可靠，但UE的抽象更贴合游戏场景需求。
> 3. **可靠性**：`Server`和`Client`可指定`Reliable`确保关键操作必达（如伤害计算），`Multicast`通常用不可靠以减少带宽。
>
>    - 另外，RPC还可以通过添加WithValidation关键字来增加验证函数的功能，这是作为检测错误数据/输入的一个手段。为了使用这个功能，需要实现`XXXFunction _Validate`与`_Implementation`函数，而原函数可以不用实现。
>    - 以下是一个例子：
>
>    ```c++
...
>                                                                                           
>    bool ServerRPCFunction_Validate( int32 Damge)
>    {
>        if ( Damge> MAX_DAGME )
>        {
...
>    }
>                                                                                           
>    void ServerRPCFunction_Implementation( int32 Damge)
>    {
>        Health -= Damge;
>    }
>    ```
...
>    
>
> Unreal内部有三种RPC机制，更详细一些如下所示：
>
> 1. 服务端调用，客户端执行。UFUNCTION中添加Client修饰符。命名规则以Client为前缀。
> 2. 客户端调用，服务端执行。UFUNCTION中添加Server修饰符。命名规则以Server为前缀。
> 3. 在服务端调用，服务端和连接上的所有客户端执行。UFUNCTION中添加NetMulticast修饰符。命名规则以Multicast为前缀。
>
> 举个例子：
>
> ```c++
...
> ```
>
> 所有的RPC函数的返回类型必须是void。这是因为RPC函数是异步的，它们在调用时并不会立即执行。相反，它们会在网络上发送一个消息，然后在目标实例（例如服务端或客户端）上执行。因为这个过程是异步的，所以RPC函数不能直接返回一个值。



### （2）什么是属性同步？应用场景，与RPC的区别是什么？

在这篇文章中也有一定的介绍：[UE4 浅谈网络同步中的RPC - 知乎](https://zhuanlan.zhihu.com/p/678112760)

> 在游戏开发中，**属性同步**（Property Replication）是网络游戏中保持多个客户端与服务器之间**对象状态一致性**的核心机制，而**RPC**（Remote Procedure Call）用于在远程机器上**触发特定逻辑**。以下是两者的定义、区别及引擎实现示例：
>
> ---
>
> ### **1. 属性同步（Property Replication）**
> #### **定义**  
> 通过自动或半自动的方式，将服务器或某个客户端的**对象属性值**（如位置、血量、分数等）实时同步到其他客户端，确保所有玩家看到相同的状态。
>
> #### **实现方式**  
> - **Unity（Netcode）**：  
>   使用 `NetworkVariable<T>` 或旧版 `[SyncVar]` 特性标记变量，当变量值变化时自动同步。  
>   ```csharp
...
>   
> - **UE（Unreal Engine）**：  
>   在 `AActor` 子类中，通过 `UPROPERTY(Replicated)` 标记属性，并在 `GetLifetimeReplicatedProps` 中声明同步规则。  
>   ```cpp
>   // .h 文件
...
> #### **应用场景**  
> - **玩家状态同步**：血量、护甲、弹药量等数值。  
> - **物理对象同步**：位置、旋转、速度（如移动的载具）。  
> - **全局游戏状态**：倒计时、比分、任务进度。  
>
> #### **特点**  
> - **自动触发**：属性值变化时自动同步，无需手动调用。  
> - **优化策略**：支持按频率（如每秒10次）或阈值（变化超过5%时同步）更新，减少带宽消耗。  
> - **单向性**：**通常由服务器向客户端同步（权威同步），客户端修改需通过RPC请求服务器。**
>
> ---
>
> ### **2. RPC（Remote Procedure Call）**
> #### **定义**  
> 在远程机器（客户端或服务器）上**执行指定函数**，用于触发事件或逻辑，例如技能释放、UI提示等。
>
> #### **实现方式**  
> - **Unity（Netcode）**：  
>   - `[ServerRpc]`：客户端调用服务器函数。  
>   - `[ClientRpc]`：服务器调用客户端函数。  
>   ```csharp
>   public class Player : NetworkBehaviour {
>       [ServerRpc]
>       void AttackServerRpc() { /* 服务器处理攻击逻辑 */ }
>                                                             
>       [ClientRpc]
>       void ShowDamageEffectClientRpc() { /* 客户端播放受击特效 */ }
>   }
>   ```
>
> - **UE（Unreal Engine）**：  
>   - `UFUNCTION(Server, Reliable)`：客户端调用服务器函数。  
>   - `UFUNCTION(Client, Reliable)`：服务器调用客户端函数。  
>   - `UFUNCTION(NetMulticast, Unreliable)`：服务器向所有客户端广播。  
>   ```cpp
>   // 客户端调用服务器
...
>
> #### **应用场景**  
> - **玩家操作反馈**：释放技能、射击、跳跃等动作需服务器验证。  
> - **事件通知**：播放特效（如爆炸）、音效、UI提示。  
> - **瞬时逻辑**：服务器权威计算伤害后，通过RPC通知客户端显示结果。
>
> ---
>
> ### **3. 属性同步 vs RPC 的核心区别**  
...
> | **目的**     | 保持状态一致性             | 触发特定逻辑                 |
> | **触发方式** | 自动（属性变化时）         | 手动调用                     |
> | **数据方向** | 单向（服务器→客户端）      | 双向（客户端↔服务器↔客户端） |
> | **带宽消耗** | 可能较高（频繁更新）       | 较低（按需触发）             |
> | **可靠性**   | 通常不可靠（可配置为可靠） | 可配置可靠/不可靠            |
> | **典型用例** | 位置、血量、得分           | 技能释放、特效播放、聊天消息 |
>
> ---
>
> ### **4. 引擎中的协作示例**  
> #### **UE中玩家受击流程**  
> 1. **属性同步**：服务器计算玩家血量变化，`Health` 属性自动同步到客户端。  
> 2. **RPC**：服务器通过 `Multicast` RPC 广播受击特效，客户端播放动画和音效。  
>
> #### **Unity中拾取道具流程**  
> 1. **RPC**：客户端调用 `[ServerRpc]` 请求拾取道具。  
> 2. **属性同步**：服务器修改道具的 `IsCollected` 属性并同步，客户端隐藏道具模型。  
>
> ---
>
> ### **总结**  
> - **属性同步**是**持续的状态同步**，适用于高频、数值型数据。  
> - **RPC**是**离散的逻辑触发**，适用于事件、动作、瞬时效果。  
> - 实际开发中两者结合使用：属性同步维护状态，RPC处理事件逻辑（如UE的 `Multicast` + 属性同步）。

关于一些业务场景当中的举例和补充，参考附录/RPC相关的markdown笔记。



### ==（3）RPC是怎么实现的？也可以以UE为例==

还是看这篇链接：[UE4 浅谈网络同步中的RPC - 知乎](https://zhuanlan.zhihu.com/p/678112760)，但暂时==虚幻用的太少了，后面看完再整理进来吧。==暂时可以答本节开始的那部分，关于RPC整体的过程。



### （4）场景题：子弹同步业务

> 玩家A发射子弹击中了玩家B，B需要扣血，同时播放特效，而另一个玩家C需要看到这个过程，怎么做？我需要看到子弹的实体，以及发射出去击中的过程。 衍生问题：子弹不同步怎么办呢，有网络延迟的话怎么保证同步？（提示：属性同步）

以下是Deepseek的回答（有进行基本的矫正）：

> 一、核心流程设计（基于权威服务器架构）：
> 1. 发射阶段：
> - 客户端A按下射击键，立即播放本地枪口特效（客户端预测）
> - 发送RPC_Fire到服务器，包含发射时间、初始位置、方向等参数
>
> 2. 服务器验证与同步：
> - 服务器校验射击合法性（CD时间、弹药量等）
> - 创建子弹实体，通过属性同步广播给所有客户端：
>   ```cpp
>   // 同步协议示例
>   class BulletSync {
>     uint32 bulletId;
...
>     Vector3 direction;
>     float speed;
>     float spawnTime; // 服务器生成时间
>   }
>   ```
>
> 3. 运动同步：
> - 服务器每100ms广播子弹位置（采用压缩的Delta同步）
> - 客户端根据接收到的参数自行计算弹道：
>   ```csharp
>   // 客户端弹道预测
>   void UpdateBullet() {
>     float elapsed = Time.time - spawnTime;
>     currentPos = startPos + direction * (speed * elapsed);
>   }
>   ```
>
> 4. 命中判定：
...
>   ```python
>   # 服务器命中处理
>   def OnBulletHit(target):
>       target.health -= damage
>       BroadcastRPC("PlayHitEffect", bulletId, hitPosition)
>       SyncAttribute(targetId, "health", target.health)
>   ```
>
> 二、同步方案组合：
> 1. 属性同步用于：
...
> ```mermaid
> sequenceDiagram
> ClientA->>Server: RPC_Fire(方向,时间戳)
> Server->>AllClients: 属性同步子弹实体
> Server->>Server: 持续检测弹道
> Server->>ClientB: 属性同步血量变化
> Server->>AllClients: RPC_PlayHitEffect
> ```
>
> 三、延迟处理方案：
> 1. 子弹不同步处理：
> - 客户端保留本地预测弹道，但接受服务器校正
> - 使用历史状态回滚：
>   ```csharp
>   // 客户端子弹修正
>   void OnBulletUpdate(ServerPos) {
>     float latency = Network.GetPing() * 0.5f;
>     Vector3 predictedPos = CalcPosition(Time.time - latency);
>     if(Vector3.Distance(predictedPos, ServerPos) > threshold){
>         StartCoroutine(SmoothCorrect(ServerPos));
>     }
>   }
>   ```
>
> 2. 命中补偿策略：
> - 服务器使用延时补偿算法，根据发射时间回溯玩家位置
> - 命中判断时参考各客户端延迟：
>   ```python
>   # 服务器延时补偿
>   def CheckHit():
>       hitTime = bullet.spawnTime + calc_flight_time()
>       for client in clients:
>           record = client.GetPositionRecord(hitTime)
>           if Raycast(record.position):
>               return True
>   ```
>
> 3. 关键属性保障措施：
> - 血量变更采用版本号控制
> - 重要事件使用可靠传输通道（TCP over UDP）
> - 客户端显示血量时添加渐变效果，但实际计算以最新同步值为准
>
> 四、异常处理补充：
> 1. 子弹失效机制：
> - 服务器设置最大存活时间（避免无限飞行）
...
> - 关键属性变更记录校验码
>
> 该方案在《守望先锋》等射击游戏中验证，可实现200ms延迟下的流畅体验。实际开发中需注意：弹道算法需完全一致、时间同步使用NTP协议、关键操作需要CRC校验。
...
>ARQ，全称“自动重传请求”（Automatic Repeat-reQuest），是计算机网络中一种保证信息可靠传输的纠错协议。 你可以把它想象成一个非常负责任的“快递员”，它的核心任务就是确保你发送的每一个数据包都能准确无误地送达目的地。
>
>**工作原理：确认与重传**
>
>ARQ协议的核心机制是**确认**和**超时重传**。
>
>*   **确认**：当接收方成功收到一个数据包后，会向发送方回送一个“确认”消息（ACK），就像签收快递一样。
>*   **超时重传**：如果发送方在一定时间内没有收到这个“确认”消息，它就会认为数据包在途中丢失了或者损坏了，于是会重新发送同一个数据包。
>
>通过这种方式，ARQ协议在不可靠的网络（比如互联网）上实现了可靠的数据传输。
>
>**ARQ的主要类型**
>
>ARQ协议主要有以下几种类型：
>
>*   **停止-等待ARQ**：这是最简单的一种。发送方每发送一个数据包，就必须停下来等待接收方的确认，收到确认后再发送下一个。这种方式虽然简单，但效率较低，因为大部分时间都可能在等待。
>*   **连续ARQ**：为了解决停止-等待协议效率低的问题，连续ARQ允许发送方连续发送多个数据包，而不需要每发一个就停下来等待确认。 这大大提高了传输效率。
>    *   **回退N帧 (Go-Back-N) ARQ**：在这种模式下，如果接收方发现收到的数据包有误或丢失，它会请求发送方从出错的那个数据包开始，重新发送之后的所有数据包。
>    *   **选择重传 (Selective Repeat) ARQ**：这是一种更优化的方式。接收方只会请求发送方重传那些真正丢失或出错的数据包，而不是全部重传。
>
>TCP协议就利用了ARQ技术，并结合滑动窗口协议，来确保数据在网络中的可靠传输。

## ==【11】TCP中的一些细节介绍，如Nagle，TIME_WAIT==

Gemini ai lab：

>TCP协议为了保证数据传输的可靠性和效率，设计了很多精妙的细节。这里我们通俗地讲解一下Nagle算法和TIME_WAIT状态。
>
>#### Nagle算法：攒够了再发，避免“零敲碎打”
>
>你可以把Nagle算法想象成一个为了节省“快递费”而存在的机制。在网络传输中，即使只发送1个字节的数据，也需要附带一个几十个字节的TCP/IP头部信息，这就像寄一件很小的东西却要用一个大箱子来包装，非常浪费。
>
>Nagle算法的作用就是避免发送过多这样的小数据包。 它的工作方式是：当有数据要发送时，如果数据量很小，它会先将这些数据“攒一攒”，等积攒到一定数量（通常是一个最大报文段长度MSS），或者等收到了之前发送数据的确认信息后，再将这些数据一次性发送出去。
>
>**优点：**
>
>*   提高了网络的利用率，减少了网络拥塞的可能性。
>
>**缺点：**
>
>*   可能会增加延迟。对于一些实时性要求很高的应用（比如在线游戏或实时操作），这种“攒一攒”的策略可能会导致明显的卡顿。 因此，TCP也提供了禁用Nagle算法的选项（TCP_NODELAY）。
>
>#### TIME_WAIT状态：确保“善始善终”的冷静期
>
>TIME_WAIT状态是TCP连接断开（四次挥手）过程中，主动关闭连接的一方会进入的一个特殊状态。 你可以把它理解为一段“冷静期”。
>
>**为什么需要TIME_WAIT状态？**
>
>主要有两个原因：
>
>1.  **保证最后的ACK报文能够到达对方**：在四次挥手的最后一步，主动关闭方会发送一个ACK报文给被动关闭方。但这个ACK报文可能会丢失。如果没有TIME_WAIT状态，主动关闭方发送完ACK后就直接关闭连接了。如果这个ACK丢失，被动关闭方会因为没收到确认而重发FIN报文，但此时主动关闭方已经“人去楼空”，无法回应，导致被动关闭方无法正常关闭。 有了TIME_WAIT状态，主动关闭方就能在这段“冷静期”内重发丢失的ACK，确保双方都能正常关闭。
>
>2.  **防止旧数据干扰新数据**：想象一下，在你和服务器断开连接后，马上又用同一个端口和服务器建立了新的连接。此时，网络中可能还游荡着上一个连接的、延迟到达的数据包。如果没有TIME_WAIT状态，这些旧数据包就可能会被新的连接误认为是新发来的数据，造成数据混乱。 TIME_WAIT状态持续的时间（通常是2倍的最大报文段生存时间，即2MSL）足以让网络中所有残余的旧数据包都消失掉，从而保证新连接的纯洁性。
>
>**对服务器的影响**
>
>对于需要处理大量短连接的服务器来说，如果服务器是主动关闭连接的一方，就会产生大量的TIME_WAIT状态，占用端口资源。 虽然TIME_WAIT状态本身不占用文件描述符和太多内存，但在高并发场景下，过多的TIME_WAIT状态仍然可能导致端口资源耗尽，影响新连接的建立。因此，在高并发服务器的调优中，如何合理地管理TIME_WAIT状态是一个重要的课题。
...
<details>
  	<summary>点击查看答案</summary>   
    <pre><code>答案：C  选项C（192.168.0.1）属于第三类私有地址。选项B（172.32.1.1）超出172.16~172.31的范围；选项D是组播地址。</code></pre> 
</details>
...
A) 32位二进制数，通常表示为四个十进制数
B) 128位二进制数，通常表示为八个十六进制数
C) 64位二进制数，通常表示为两个十进制数
D) 256位二进制数，通常表示为十六进制数



<details>
  	<summary>点击查看答案</summary>   
    <pre><code>A) 32位二进制数，通常表示为四个十进制数</code></pre> 
</details>

**讲解:** IPv4地址由32位二进制数构成，通常以四个十进制数表示，每个数的范围是0到255，格式为x.x.x.x（例如：192.168.1.1）。而IPv6地址则是128位的，通常以八个十六进制数表示。
...
引用某个 IPv4地址时，可使用 W.X.Y.Z 的点分十进制表示形式，如 192.168.1.10 等。
...
好的！我来详细解释这两个问题，尽量用简单易懂的方式帮助你理解。

---

### **一、IPv4地址的格式**
IPv4地址是互联网中用于唯一标识设备的32位二进制数，通常用**点分十进制**表示。以下是它的核心要点：

#### **1. 格式规则**
- **二进制形式**：由32位0或1组成，例如：  
  `11000000 10101000 00000001 00000001`
- **点分十进制形式**：将32位分成4组（每组8位），转换为十进制，用`.`分隔，例如：  
  `192.168.1.1`

#### **2. 结构解析**
- **每段范围**：每段8位二进制数，十进制范围是 **0~255**。  
  例如：`256`是无效的（因为8位最大是`11111111`=255）。
- **完整示例**：  
  `172.16.254.1` → 二进制为：  
...
- **私有地址**（用于局域网，不可直接访问公网）：  
  ...
  https://blog.csdn.net/sz_bdqn/article/details/42970505
- **环回地址**（测试本机网络）：`127.0.0.1`  
- **广播地址**：`255.255.255.255`  
- **子网掩码**：如`255.255.255.0`，用于划分网络和主机部分。

#### **4. 为什么需要IPv4？**
- **唯一性**：确保每个设备在网络中有唯一标识。  
- **路由**：帮助路由器将数据包发送到正确的目标。
...
>当有Early-Z（早期深度测试）时，顺序变为：
>
>**深度/模板 测试 -> Alpha 测试**
>
>**一句话解释：**
>GPU会先进行**深度和模板测试**，如果片元在这里就被剔除了，就根本不会去运行片元着色器，因此也**不会进行Alpha测试**。只有通过了深度和模板测试的片元，才有机会进入片元着色器中进行Alpha测试。
...
> 1. **骨骼动画**  
>
>    - **层级变换**：骨骼以树状结构组织，每块骨骼对应局部变换矩阵（平移、旋转、缩放）（在关节空间，相对于父骨骼的变换，此变换矩阵可以从子关节空间转换到父关节空间）。父骨骼变换驱动子骨骼运动，通过矩阵级联（`父矩阵 × 子矩阵`）计算全局变换（**由关节空间转换到模型空间**）。  $\text{GlobalMatrix}_i$
...
>      $$
>      
>      - $$P_{\text{original}}$$ -顶点在模型空间中的原始位置
>      - **顶点从模型空间到骨骼局部空间**
>        `InverseBindMatrix_i · P_original`：将顶点坐标转换到骨骼 `i` 的局部空间（绑定姿势下）。
>      - **应用骨骼当前动画变换**
>        `GlobalMatrix_i · (InverseBindMatrix_i · P_original)`：将顶点从骨骼 `i` 的局部空间转换到当前动画帧的模型空间。
...
> >#### **(1) `P_original`：原始顶点位置**
> >
> >- **绑定姿势（Bind Pose）**：模型未动画时的初始状态，顶点坐标基于此姿势定义。
> >- **顶点坐标**：`P_original` 是顶点在绑定姿势下的局部坐标（模型空间）。
> >
> >#### **(2) `InverseBindMatrix_i`：逆绑定矩阵**
> >
> >- **作用**：将顶点从模型空间转换到骨骼 `i` 的**局部空间**。
> >- **原理**：在绑定姿势下，骨骼 `i` 的变换矩阵为 `BindMatrix_i`（骨骼的模型空间到局部空间的变换）。`InverseBindMatrix_i` 是它的逆矩阵，即 `BindMatrix_i⁻¹`。
> >- **示例**：若骨骼在绑定姿势下位于模型空间的 (1, 0, 0)，则 `BindMatrix_i` 包含平移 (1, 0, 0)，而 `InverseBindMatrix_i` 会将顶点坐标平移 (-1, 0, 0)。
> >
> >#### **(3) `GlobalMatrix_i`：骨骼的全局变换矩阵**
> >
> >- **作用**：表示骨骼 `i` **在当前动画帧下的全局变换**（模型空间中的位置、旋转、缩放）。
> >
> >- 层级计算
> >
> >  ：骨骼的全局矩阵由父骨骼链的变换逐级相乘得到。例如：
> >
> >  ```
> >  GlobalMatrix_i = ParentGlobalMatrix × LocalMatrix_i
> >  ```
> >
> >  LocalMatrix_i是骨骼**相对于父骨骼的局部变换（由动画关键帧数据驱动）。**
> >
> >  

![image-20250324163507305](assets/image-20250324163507305.png)

将顶点坐标转换 从模型空间 ----------->到骨骼 `i` 的局部空间/关节空间（绑定姿势下）----------------------->当前动画帧的模型空间
...
## 【9】如何判断点在多边形内？写一个判断点在三角形内的代码 :new:


### 🌟 一、核心原理（先看动画想象）
想象三角形是用三条篱笆围成的院子，**点P就是一只小鸡**。我们要判断这只小鸡是否在院子内。

**判断方法**：  
小鸡必须同时在三道篱笆的**同一侧**（比如全在左侧或全在右侧）。若有一道篱笆把鸡隔到不同侧，鸡就在院子外。

——
### 📐 二、什么是叉乘？如何判断方向？
**叉乘**在二维中可以简单理解为**方向判断器**。给定两个向量，叉乘的正负号告诉我们它们的相对方向。

**公式**：  
对于向量 `AB = (Bx - Ax, By - Ay)` 和向量 `AP = (Px - Ax, Py - Ay)`，叉乘值为：  
`叉乘 = (Bx - Ax) * (Py - Ay) - (By - Ay) * (Px - Ax)`
...
**计算过程**：  
1. **cross1**：检查P在边AB的哪一侧  
2. **cross2**：检查P在边BC的哪一侧  
3. **cross3**：检查P在边CA的哪一侧  

**返回条件**：  
如果三个叉乘的符号**全为非负**（≥0）或**全为非正**（≤0），说明P在所有边的同一侧，即内部。
...
1. **计算cross1（边AB）**  
   - 向量AB = (1-0, 0-0) = (1,0)  
   - 向量AP = (0.3-0, 0.3-0) = (0.3,0.3)  
   - 叉乘 = 1*0.3 - 0*0.3 = **0.3（正）** → P在AB左侧  

2. **计算cross2（边BC）**  
   - 向量BC = (0-1,1-0) = (-1,1)  
   - 向量BP = (0.3-1,0.3-0) = (-0.7,0.3)  
   - 叉乘 = (-1)*0.3 - 1*(-0.7) = -0.3 + 0.7 = **0.4（正）** → P在BC左侧  

3. **计算cross3（边CA）**  
   - 向量CA = (0-0,0-1) = (0,-1)  
   - 向量CP = (0.3-0,0.3-1) = (0.3,-0.7)  
   - 叉乘 = 0*(-0.7) - (-1)*0.3 = 0 + 0.3 = **0.3（正）** → P在CA左侧  

**结果**：三个叉乘均为正，符号一致 → **点P在内部**。

——
### 🚨 五、注意事项
1. **顶点顺序要一致**：  
   输入顶点A、B、C时需按顺时针或逆时针顺序排列，否则可能导致判断错误。

2. **点在边上如何处理**：  
   代码中 `>=0` 和 `<=0` 包含等于零的情况，即点在边上也算内部。若需严格内部，可改为 `>` 和 `<`。

3. **浮点数精度问题**：  
   实际计算时，叉乘可能因精度误差接近零但不严格为零，需根据场景调整容差。
...
### 七、动手尝试
1. **修改测试点**：  
   将 `P` 改为 `(1,1)`，观察输出是否变为“点在外面”。

2. **调整顶点顺序**：  
   将顶点顺序改为 `A(0,0)`, `C(0,1)`, `B(1,0)`，看看是否影响结果。

```C++
bool isInsideTri(Point A,Point B,Point C,Point P)
{
	//cross = ax*by - bx*ay
	double cross1 = (B.x - A.x) * (P.y - A.y) - (B.y - A.y) * (P.x - A.x); //AB PA
	double cross2 = (C.x - B.x) * (P.y - B.y) - (C.y - B.y) * (P.x - B.x);//BC PB
	double cross3 = (A.x - C.x) * (P.y - C.y) - (A.y - C.y) * (P.x - C.x);//CA PC
...
D:\PGPostgraduate\githubNotePrepareForWork\PrepareForWorkNotes\计算机图形学\剔除相关\lesson_21剔除技术\剔除技术（可见性判断）.md

## 【12】移动端渲染管线 的瓶颈在哪里，如何性能优化 :dragon_face: :astonished:  :sun_behind_large_cloud: 

D:\PGPostgraduate\githubNotePrepareForWork\PrepareForWorkNotes\计算机图形学\Unity 渲染 引擎相关\移动端渲染管线的瓶颈 性能优化.md

## 【13】为什么要减少Draw Call?CPU-GPU 通信会比较慢吗? 为什么这么说? 慢在哪里？:dragon_face: :sun_behind_large_cloud: 
...
## ==【1】Navigation系统的介绍== :sun_behind_large_cloud: 





## ==【2】Mono和IL2CPP的区别（参考文章还没看）== :sun_behind_large_cloud: 
...
- **回调模式**：
  ...
  void Start() {
      FetchData((data) => { Debug.Log(data); });
  }
  ```

  - **劣势**：多层回调导致嵌套（回调地狱），可读性差。
...
## 【6】Unity物理引擎的碰撞检测相关 :sun_behind_large_cloud: 
...
- （1）正确的传送代码（不要拿出transform直接赋值！！直接改position的值会比较好）：
...
## 【9】 GPU compute shader 相关内容 :cat: :sun_behind_large_cloud: 
...
https://docs.unity3d.com/cn/2018.4/Manual/ScriptableRenderPipeline.html

可编程渲染管线 (Scriptable Render Pipeline, SRP) 是 Unity [内置渲染管线](https://docs.unity3d.com/cn/2018.4/Manual/SL-RenderPipeline.html)的替代方案。 使用 SRP 可以通过 C# 脚本控制和定制渲染方式。这样，您可以稍微修改或完全构建并自定义渲染管线以满足您的需求。

SRP的类型

Unity提供了两种主要的SRP：

- **高清渲染管线（HDRP）**：适用于高端平台，支持高级图形特性，如物理光照、体积雾等。
- **轻量级渲染管线（LWRP）**（现已更名为Universal Render Pipeline，URP）：适用于各种平台，特别是移动设备，提供良好的性能和视觉效果平衡。



可以提到项目实现过RendererFeature：扭曲，像素，径向模糊，黑白等

  Unity中的RendererFeature是一个允许开发者在渲染管线中添加自定义渲染效果的功能。是一种自定义和扩展Unity的[URP](https://zhida.zhihu.com/search?content_id=238298538&content_type=Article&match_order=1&q=URP&zhida_source=entity)（通用渲染管线）或者[HDRP](https://zhida.zhihu.com/search?content_id=238298538&content_type=Article&match_order=1&q=HDRP&zhida_source=entity)（高清渲染管线）的方式。开发者可以通过编写自定义的RendererFeature来实现特殊的图像效果，如后处理效果、特殊光照效果等。



## 【12】Unity的各个文件夹的作用

### （1）StreamingAssets文件夹的作用

> ### **Unity的StreamingAssets文件夹**  
>
> #### **作用**  
> 1. **存放只读资源**：用于存储需随包体分发但**运行时动态加载**的文件（如配置表、视频、音频、AssetBundle等）。  
> 2. **跨平台兼容**：路径通过 `Application.streamingAssetsPath` 统一访问，无需处理平台差异。  
>
> #### **使用方式**  
> ```csharp
> // 加载文件（需配合WWW/UnityWebRequest）  
> string filePath = Path.Combine(Application.streamingAssetsPath, "data.json");  
> string jsonData = File.ReadAllText(filePath); // 或使用UnityWebRequest异步加载  
> ```
>
> #### **关键特性**  
> - **不压缩**：资源原样打包（与Resources不同）。  
> - **不可写**：仅读取，修改需复制到 `Application.persistentDataPath`。  
> - **平台路径差异**：  
>   - **Android/iOS**：资源在只读包内（需用 `UnityWebRequest` 访问）。  
>   - **PC**：直接文件路径访问。  
>
> #### **典型用途**  
> - 初始配置（JSON/XML）、AB包、视频/音频等大文件。



## 【13 】MonoBehaviour函数调用顺序 

https://blog.csdn.net/windeer/article/details/51510529
...
## ==【1】设计一个背包系统（写代码）考虑各种情况，如空位置、加物品超上限== :sun_behind_large_cloud: 
...
在Algos——Leetcode文件夹中有所有HOT 100的代码，这里有一些HOT 100的题就直接给出代码了。
...
https://blog.csdn.net/m0_38013346/article/details/80434208

<img src="assets/image-20250730194228602.png" alt="image-20250730194228602" style="zoom:67%;" />
...
push的时候，push，然后将size-1个元素循环放在末尾
...
pop ，top的时候将size-1个元素放到队尾，top的时候记得把最后一个再放到队尾
...
理解了快速选择算法之后，可以做一下这道题目：[215. 数组中的第K个最大元素 - 力扣（LeetCode）](https://leetcode.cn/problems/kth-largest-element-in-an-array/description/)，代码如下：（这个暂时不用看）
...
void quickSort(vector<int>&vec, int l, int r)
{
    ...
    while(i<j) //注意 没有等号的 否则会超时，因为等于之后就不会再移动了。因为结束条件必须是i>=j
    {
    ...
    quickSort(vec,l,j);
    quickSort(vec,j+1,r); //这个必须是j 不可是(l+r)>>1
}
...
public:
    int fastSelect(vector<int>& nums, int k, int l, int r){
        ...
        while(i<j)//！！ 不可以是等号 是因为如果<=，假设此时i=0，j=0，接下来 do j--; 【j=-1】while(nums[j]>x);报错
        {
            do i++; while(nums[i]<x); //有无等号都可过 性能不知道一不一样
            do j--; while(nums[j]>x);
...
>不可是midIdx  不要和二分混了。。。 快速选择/快排中这个中心元素（划分元素）的位置会改变的！！！
>
>以下是错误的：
>
>![image-20250318131631567](assets/image-20250318131631567.png)
>
>在快速选择算法中，分区操作后的实际分割点由 `j` 决定，而非初始的中间索引 `midIdx`。原代码错误地使用 `midIdx` 划分递归区间，导致区间范围不正确。以下通过示例说明问题：
>
>**示例分析**  
>假设数组为 `[5,2,3,1,4]`，寻找第3大元素（即3）。  
>
>1. **初始调用**：`l=0`, `r=4`, `midIdx=2`（值为3）。  
>2. **分区过程**：  
>   - 选择 `x = nums[midIdx] = 3`。  
>   - 分区后数组变为 `[5,3,2,1,4]`，实际分割点 `j=1`。  
>3. **递归调用判断**：  
>   - `k=2`（对应第3大元素）应位于右半部分 `j+1=2` 到 `r=4`。  
>   - **原代码错误**：递归到 `midIdx+1=3` 到 `4`（即区间 `[1,4]`），错过正确元素。  
>   - **正确逻辑**：递归到 `j+1=2` 到 `4`（即区间 `[2,4]`），正确缩小范围。
>
>**结论**  
>分区后的分割点由 `j` 确定，递归区间应基于 `j` 划分。修正后的代码应使用 `j` 而非 `midIdx`：
>
>```cpp
>if (k <= j) return fastSelect(nums, k, l, j);
>else return fastSelect(nums, k, j+1, r);
...
public:
    void heapify(vector<int>& nums, int n, int i){ //堆一共需要n个节点,从第i个开始调整
        ...
        if(max!=i){
            ...
            heapify(nums, n, max);//递归的走左孩子或者右孩子方向
        }
    }
...
## 【15】手撕LRU算法 :astonished:

面试被这题肘翻了

LRU相关，给一些数据项，计算缓存命中次数，计算最后缓存中即将被淘汰的数据项。这道题目是**Leetcode Hot 100**中的题目，应当熟练掌握。
...
这道题目使用C++来写的话，代码如下（重点是理解好双向链表的头插、删除某个节点以及删除末尾节点，C++中通过添加`virtual head`与`virtual tail`来实现）
...
## 【23】 [230. 二叉搜索树中第 K 小的元素](https://leetcode.cn/problems/kth-smallest-element-in-a-bst/):dog:
...
思考题：如果求第k大，那么将遍历顺序从左中右改成右中左即可。

> 补充题目：[98. 验证二叉搜索树](https://leetcode.cn/problems/validate-binary-search-tree/)，具体做法记录在HOT 100对应的笔记当中。



## 【24】m个技能点数，n个技能，每个技能有对应的学习点数要求以及技能伤害，问最大伤害值是多少？
...
std::mutex mtx;
bool is_odd = true;
void PrintOdd() //odd是奇数
{
    for (int i = 1; i <= 10; i += 2)
    {
        std::unique_lock<std::mutex> lock(mtx);//加锁
        cv.wait(lock, [] {return is_odd; });//wait的时候释放锁
        std::cout << "t1:"<<i << std::endl;
        is_odd = false;
        cv.notify_all();//会唤醒所有等待队列中阻塞的线程
    }
}
void PrintEven() //even是偶数
{
    for (int i = 2; i <= 10; i+=2)
    {
        std::unique_lock<std::mutex> lock(mtx);
        cv.wait(lock, []{return !is_odd;});
...
https://leetcode.cn/problem-list/concurrency/

### std::condition_variable C++11

#### 步骤1：`unique_lock<mutex> lk(mtx)`

- **作用**：立即锁定互斥量`mtx`

- 原理：

  - `unique_lock`是一个智能锁管理器
  - 构造时自动调用`mtx.lock()`
  - 析构时（比如离开作用域）自动调用`mtx.unlock()`
...
#### 步骤2：`cv.wait(lk, predicate)`

- **内部工作机制**（原子操作）：

  1. **释放锁**：`lk.unlock()`（允许其他线程获得锁）

  2. **进入等待**：线程挂起，直到被`cv.notify_*`唤醒

  3. **重新上锁**：被唤醒后自动执行`lk.lock()`

  4. 检查条件：

     - 如果`predicate()`返回`true` → 继续执行
  - 如果返回`false` → 回到步骤1重新等待
...
public:
    int hammingWeight(int n) {
        //return __builtin_popcount(n); //OK的
...
> 注：还有一种题目是给出中序遍历和后序遍历序列，求前序遍历，方法是类似的，但做的时候一定要耐心、小心。
...
对应力扣有这道题：[面试题 17.19. 消失的两个数字 - 力扣（LeetCode）](https://leetcode.cn/problems/missing-two-lcci/description/)
...
> 1. **计算理想总和与实际总和**：
>
>    - 理想总和 $S = \frac{10000 \times 10001}{2} = 50005000$。
>    - 实际总和 $s = \sum \text{给定数组}$。
>    - 缺失两数之和 $ x + y = S - s$。
>
> 2. **计算理想平方和与实际平方和**：
>
>    - 理想平方和 $Q = \frac{10000 \times 10001 \times 20001}{6} \approx 333,383,335,000$。
>    - 实际平方和 $q = \sum (\text{给定数组元素}^2)$。
>    - 缺失两数平方和 $x^2 + y^2 = Q - q$。
>
> 3. **联立方程求解**：
>
>    - 设 $ x + y = a $，$ x^2 + y^2 = b$。
>
>    - 利用 $(x + y)^2 = x^2 + y^2 + 2xy$，得：
>      $$
>      xy = \frac{a^2 - b}{2}
>      $$
>      
>
>    - 解二次方程 $t^2 - at + \frac{a^2 - b}{2} = 0$，根即为缺失数  x  和  y 。**注意:这一步基于韦达定理：如果x和y是某个二次方程的根，则有：$t^2-(x+y)t+xy=0$**
>
> 韦达定理的介绍：[韦达定理_百度百科](https://baike.baidu.com/item/韦达定理/105027)，其实想一想就是(t-x)(t-y)=0展开后的式子。
>
> #### **代码示例（Python）**：
>
> ```python
...
> 
>     s = sum(arr)
>     q = sum(x*x for x in arr)
> 
>     a = S - s  # x + y
>     b = Q - q  # x² + y²
> 
>     # 解方程 t² - a*t + (a² - b)/2 = 0
>     discriminant = a * a - 2 * (a * a - b)
>     sqrt_discriminant = int(discriminant ** 0.5)
>     x = (a - sqrt_discriminant) // 2
>     y = a - x
> 
>     return x, y
> ```
...
> #### **注意事项**：
>
> - **数值溢出**：在编程时需使用大整数类型（如Python的`int`），避免溢出。
> - **精度问题**：平方根计算需确保精确，避免浮点误差。
>
> ---
>
> ### **方法二：位运算（异或分组法）**
...
>
> 1. **计算异或结果**：
>    - 对1到10000和给定数组所有元素进行异或，结果 $xor = x \oplus y$。
>
> 2. **找到差异位**：
>    - 找到 \( xor \) 中任意一个为1的二进制位（如最右侧的1），记为 mask。该位表示  x 和  y 在此位不同。
>
> 3. **分组异或**：
>    - 将1到10000的数及给定数组分为两组：
>      - 第1组：该位为0的数。
...
>
> ```python
> def find_missing_numbers_xor(arr):
>     n = 10000
>     xor_total = 0
...
>
> 1. **标记存在状态**：
>    - 创建长度为10001的布尔数组 `exists`，初始化为`False`。
>    - 遍历给定数组，将存在的数对应位置标记为`True`。
>
> 2. **遍历查找缺失数**：
>    - 遍历1到10000，找到未被标记的两个位置。
>
> #### **代码示例（Python）**：
>
> ```python
> def find_missing_boolean(arr):
>     exists = [False] * 10001
>     for num in arr:
>         exists[num] = True
> 
...
> | 方法           | 时间复杂度 | 空间复杂度 | 适用场景                     |
> | -------------- | ---------- | ---------- | ---------------------------- |
> | **数学公式法** | \( O(n) \) | \( O(1) \) | 数值不溢出时最优，需处理大数 |
> | **位运算法**   | \( O(n) \) | \( O(1) \) | 无精度问题，代码稍复杂       |
> | **布尔数组法** | \( O(n) \) | \( O(n) \) | 简单直接，适合n较小情况      |
>
> - **推荐方法**：优先选择**位运算法**或**数学公式法**，满足高效且低空间需求。
> - **实际应用**：根据编程语言和数值范围选择合适方法，注意处理大数问题。
...
拆箱是从 `object` 类型到[值类型](https://learn.microsoft.com/zh-cn/dotnet/csharp/language-reference/builtin-types/value-types)或从接口类型到实现该接口的值类型的显式转换。 拆箱操作包括：
...
- 相对于简单的赋值而言，装箱和拆箱过程需要进行**大量的计算**。 对值类型进行装箱时，必须分配并构造一个新对象。 
- 拆箱所需的强制转换也需要进行大量的计算，只是**程度较轻**。 有关更多信息，请参阅[性能](https://learn.microsoft.com/zh-cn/dotnet/framework/performance/performance-tips)。 
- 装箱和取消装箱过程需要进行大量的计算。 对值类型进行装箱时，必须创建一个全新的对象。 这可能比简单的引用赋值用时最多长 20 倍。 拆箱的过程所需时间可达赋值操作的四倍。

**装箱与拆箱的应用场景：**

- 我们的项目中：
  - UI框架，会有各种类型的Message，在Open函数的时候传入的对象会使用装箱操作，在打开的特定UI View中的OnOpen函数中进行拆箱操作（因为每个UI View自己知道具体的Message是什么类），从而实现解耦。（待确认，但基本没问题）
...
> - `Queue<T>`：循环数组：数组实现，FIFO，自动扩容；[Queue 类 (System.Collections) | Microsoft Learn](https://learn.microsoft.com/zh-cn/dotnet/api/system.collections.queue?view=net-8.0)。==后面有时间再深入了解一下C#中的各种容器，看了一下Queue确实是有自动扩容的，默认增长因子是2倍，至少是4个==
...
在微软的.Net框架中，任何语言只有能编译成IL后才能被CLR执行，也就是说C#需要一个编译器来编译为IL，还需要有一个CLR来负责翻译IL，Unity选择的是Mono，Mono是另一家公司做的跨平台的开源.Net FrameWork实现，但后面Mono很久没维护了，Unity已经把IL2CPP作为其核心了。下图是C#,IL,CLR之间的关系：
...
>
> #### **1. 动态编译的本质**
> - **按需编译**：JIT编译器不会在程序启动时编译所有代码，而是**首次调用某个方法时**才将其从IL（Intermediate Language）编译为当前设备的本地机器码（如x86、ARM指令集）。
> - **举例**：  
>   假设游戏中有一个`Player.Update()`方法，在玩家第一次操控角色时（即第一次调用该方法），CLR才会触发JIT编译器将其IL代码编译为机器码。后续调用直接使用已编译的机器码，无需重复编译。
>
> ---
>
> #### **2. Unity游戏中的文件与编译阶段**
> - **xxx.dll文件的内容**：  
>   Unity使用C#编写逻辑代码，编译后生成的`.dll`文件包含**IL代码**（非机器码）。这些DLL在游戏启动时被CLR加载到内存中，但此时代码仍是IL形式。
> - **编译阶段**：  
>   当游戏运行时，CLR按以下流程处理：  
>   1. **加载IL代码**：从DLL中读取IL指令。  
>   2. **触发JIT编译**：某个方法首次被调用时，JIT编译器将其IL代码转换为机器码。  
>   3. **执行机器码**：编译后的机器码直接由CPU执行。  
>   4. **缓存机器码**：已编译的方法存入内存，后续调用跳过编译直接执行。
>
> ---
>
> #### **3. 具体过程示例**
> 以Unity游戏中的一个技能释放方法`Skill.Cast()`为例：  
> 1. **游戏启动**：  
>    - `Skill.dll`中的IL代码被加载到内存，但未编译。  
> 2. **玩家首次释放技能**：  
>    - CLR检测到`Skill.Cast()`方法未被编译，触发JIT编译器。  
>    - JIT读取`Skill.Cast()`的IL代码，生成针对当前CPU架构的机器码（如x86-64）。  
>    - 编译完成后，机器码存入内存并执行。  
> 3. **后续释放技能**：  
>    - 直接调用已编译的机器码，无编译开销。
>
> ---
>
> #### **4. 跨平台差异（Unity的JIT与AOT）**
> - **常规平台（PC、Android）**：  
>   使用标准JIT编译，IL代码在运行时动态编译。  
> - **iOS等限制平台**：  
>   - **禁用JIT**：因苹果安全策略，iOS不允许动态代码生成。  
>   - **改用AOT（Ahead-Of-Time）**：通过IL2CPP工具将IL代码转换为C++代码，再编译为机器码打包进游戏。  
>   - **结果**：所有代码在安装时已编译为机器码，运行时无JIT过程。
>
> ---
>
> #### **5. 性能权衡**
> - **优点**：  
>   - **启动速度快**：无需预先编译全部代码。  
>   - **运行时优化**：JIT可根据当前设备CPU特性优化代码（如使用AVX指令集）。  
> - **缺点**：  
>   - **首次调用延迟**：编译可能导致短暂卡顿（如进入新场景时）。  
>   - **内存开销**：需缓存编译后的机器码。
>
> ---
>
> #### **6. 验证方法**
> - **查看程序集代码**：  
>   使用调试工具（如WinDbg）附加到游戏进程，找到`Skill.Cast()`方法的内存地址，可看到其内容为JIT生成的机器码（如`55 48 8B EC`等十六进制指令）。  
> - **Unity Profiler**：  
>   在性能分析中观察“脚本编译”耗时，首次调用方法时会出现峰值。
>
> ---
>
> ### **总结**
> JIT的“动态性”体现在**按需编译**，即方法首次调用时生成机器码。Unity游戏的DLL存储IL代码，运行时由CLR动态编译，而iOS等平台通过IL2CPP转为静态编译以绕过限制。
...
void func(string s)
{
    s = "zzz"; //这样是错的，无法达到目的，解决方案：形参+ref/out
}
```



注：在C++篇的【34】题中有整理与C++的`string`原理的相关问题，可以顺便进行复习。
...
有可能会影响业务的逻辑，同时效率可能会很低。

此时如果Equals写对了，但是GetHashCode写的不好：

- 如果GetHashCode是合理但是低效的，**此时GetHashCode可能会导致很多不同的对象判断HashCode相等，导致还要额外调用很多次Equals判断，造成性能下降。**举例来说，极端情况下`GetHashCode`直接return 0，此时运行效率会降低。
...
- Equals如果写错了，很可能会造成较大的问题（相同对象判断为不同，或者不同对象判断为相同从而覆盖）



## 【8】C++的内存分布与C#有什么不同

> 以下是 C++ 与 C# 内存分布的核心区别对比：
>
> ---
>
> ### **1. 内存布局基础**
> #### **C++**
> - **完全手动控制**：内存分为 **栈（Stack）、堆（Heap）、全局/静态存储区**。
>   - **栈**：存储局部变量、函数参数，自动分配/释放（LIFO）。
>   - **堆**：通过 `new/delete` 手动管理动态内存，生命周期由开发者控制。
>   - **全局区**：全局变量、静态变量，程序启动时分配，结束时释放。
> - **内存模型贴近硬件**：直接操作物理内存地址（指针）。
>
> #### **C#**
> - **托管内存模型**：内存分为 **栈（Stack）、托管堆（Managed Heap）**。
>   - **栈**：存储值类型变量（如 `int`、`struct`）和方法调用上下文。
>   - **托管堆**：存储引用类型对象（如 `class`），由 **GC（垃圾回收器）** 自动管理。
>   - **非托管堆**：通过 `unsafe` 代码手动分配（类似 C++），但极少使用。
>
> ---
>
> ### **2. 内存分配与释放**
> | **特性**         | **C++**                                   | **C#**                                      |
> | ---------------- | ----------------------------------------- | ------------------------------------------- |
> | **动态内存分配** | 显式使用 `new/delete`，需手动管理生命周期 | 隐式通过 `new` 分配，GC 自动回收            |
> | **内存泄漏风险** | 高（忘记 `delete` 导致泄漏）              | 低（GC 自动回收）                           |
> | **内存访问安全** | 直接操作指针，可能越界或悬垂指针          | 引用类型受 GC 保护，`unsafe` 代码需显式启用 |
> | **内存碎片**     | 频繁分配/释放可能导致堆碎片               | 托管堆通过 **分代压缩** 减少碎片            |
>
> ---
>
> ### **3. 对象生命周期管理**
> #### **C++**
> - **确定性析构**：通过析构函数（`~ClassName()`）和 **RAII**（资源获取即初始化）管理资源。
> - **示例**：
>   ```cpp
>   class FileHandler {
>   public:
>       FileHandler() { file = fopen("data.txt", "r"); }
>       ~FileHandler() { fclose(file); } // 析构时自动释放资源
>   private:
>       FILE* file;
>   };
>   ```
>
> #### **C#**
> - **非确定性回收**：依赖 GC 的 **分代回收算法**（Gen 0/1/2）自动回收内存。
> - **资源释放**：通过 `IDisposable` 接口和 `using` 语句实现确定性释放：
>   ```csharp
...
>
> #### **C#**
> - **引用安全**：引用类型通过托管引用访问，GC 跟踪对象可达性。
>   ```csharp
>   object obj = new object(); // 托管堆分配，obj 是引用
>   ```
> - **边界检查**：数组访问自动检查索引有效性（抛出 `IndexOutOfRangeException`）。
>
> ---
>
> ### **5. 典型代码对比**
...
>
> ### **6. 性能与场景对比**
> - **C++**：内存控制灵活，适合高性能场景（如游戏引擎、实时系统），但开发成本高。
> - **C#**：内存安全高，开发效率高，适合业务系统，但 GC 可能导致短暂停顿（可通过 **GC 调优** 缓解）。
>   - 关于GC调优可以参考这一篇：[笔记：如何优化垃圾回收（GC）的性能_尽量减少大对象的分配,并重用对象或使用对象池(如 sync.pool),以降低垃圾回收的压-CSDN博客](https://blog.csdn.net/u010975589/article/details/141717910)，其实自己说的话可以提到**对象池/内存池的管理**，以及有时用struct替代class（从而在栈上解决问题），还有关键代码段禁用GC。
>
>
> ---
>
> ### **总结**
> - **C++**：手动管理内存，贴近硬件，性能极致但风险高。
> - **C#**：自动托管内存，安全性强，牺牲部分灵活性换取开发效率。
...
![image-20250729221254516](assets/image-20250729221254516.png)
...
- 阶段1: Mark-Sweep 标记清除阶段，先假设heap中所有对象都可以回收，然后找出不能回收的对象，给这些对象打上标记（绿色（黄色）），最后heap中没有打标记的对象都是可以被回收的；
...
> **注意**，GC只能释放托管资源，不能释放非托管资源。非托管资源包含数据库连接、文件流等。对于非托管资源的释放需要实现`IDispose`接口。如果不想实现Dispose方法又想自动释放非托管资源，可以用析构函数。:cat2:
...
> Unity使用增量式GC：https://docs.unity3d.com/cn/current/Manual/performance-garbage-collector.html
>
> ##### **增量式GC vs 传统GC**
>
> - **传统GC（Stop-the-World）**：一次性遍历所有内存对象，标记并清理无用对象，导致主线程暂停（卡顿）。
> - **增量式GC**：将GC任务拆分为多个小步骤，穿插在游戏帧更新之间执行，避免长时间阻塞主线程。



## 【10】接口与抽象类的区别

https://www.nowcoder.com/discuss/522293249537507328

### **一、接口（Interface）与抽象类（Abstract Class）的区别**

#### **1. 核心定义**
| **特性**       | **接口（Interface）**                                | **抽象类（Abstract Class）**   |
| -------------- | ---------------------------------------------------- | ------------------------------ |
| **方法实现**   | 所有方法默认抽象，无具体实现（C# 8.0起允许默认实现） | 可以包含抽象方法和具体实现方法 |
| **多继承支持** | 一个类可实现多个接口                                 | 一个类只能继承一个抽象类       |
| **成员变量**   | 不能包含成员变量（C#中接口不能有字段）               | 可以包含字段、属性、方法等     |
| **构造函数**   | 无                                                   | 有                             |
| **设计目的**   | 定义行为契约（"Can-Do"关系）                         | 提供部分公共实现（"Is-A"关系） |
...
- **接口使用场景**：
  ...
  ```
  - **优势**：跨类型对象共享行为（如`IUpdatable`用于每帧更新）
...
- **抽象类使用场景**：
  ...
  public abstract class Character {
      ...
      public abstract void UseSkill(); // 需要子类实现
  }
...
#### **3. 关键选择原则**
- **使用接口**：需要多继承、定义跨层级行为契约时。
- **使用抽象类**：需要提供基础实现、限制单一继承时。

#### **接口与抽象类问题**

**回答示例：**
"在游戏开发中，接口用于定义跨类型对象的行为契约。例如，当我们需要让玩家、NPC、甚至场景道具都具备可交互能力时，可以定义`IInteractable`接口，包含`OnInteract()`方法。

而抽象类更适合为同一类对象提供基础实现，比如所有`Enemy`子类共享巡逻、追击等通用AI逻辑，通过抽象类`EnemyBase`的`Patrol()`方法实现代码复用，同时要求子类必须实现`Attack()`抽象方法。"
...
| **特性**         | **C# 结构体**                                 | **C++ 结构体**                                               |
| ---------------- | --------------------------------------------- | ------------------------------------------------------------ |
| **类型系统**     | 值类型（栈/堆内联分配）                       | 用户定义的值类型（默认`public`访问，语法与类类似）           |
| **继承**         | ❌ 不支持继承                                  | ✔️ 支持继承和多态（通过基类或虚函数）                         |
| **接口实现**     | ✔️ 可继承接口                                  | ❌ 无接口概念（可用抽象类模拟）                               |
| **构造函数**     | ❌ 不能定义无参构造函数 字段自动初始化为默认值 | ✔️ 可自定义无参/带参构造函数 成员变量默认未初始化（需手动初始化） |
| **内存管理**     | ✔️ 自动垃圾回收（无显式析构）                  | ✔️ 支持手动内存管理（如`new`/`delete`） 可定义析构函数        |
| **默认复制行为** | 传递时**值复制**（大结构体需注意性能）        | 默认值复制，但可通过`&`引用或`*`指针避免复制                 |
| **Nullable支持** | ✔️ 需用`Nullable<T>`或`T?`表示空值             | ✔️ 通过指针（`T*`）或`std::optional<T>`处理                   |
| **多态性**       | ❌ 不支持虚函数/多态                           | ✔️ 支持虚函数和运行时多态                                     |
| **适用场景**     | 轻量级数据（如坐标、颜色） 避免频繁装箱       | 灵活内存控制、性能敏感场景 需与C兼容的数据结构               |

> 注：更多的细节有需要阅读一下C#相关的书籍再来补充吧。
...
有关信息论，可以阅读这篇文章：[(99+ 封私信 / 82 条消息) 1000桶水，其中一桶有毒，猪喝毒水后会在15分钟内死去，想用一个小时找到这桶毒水，至少需要几头猪？ - 知乎](https://www.zhihu.com/question/60227816/answer/1274071217)（还没有看完，刚看了一点点，==信息论的水很深，有时间再细看吧，3B1B也有对应的视频，这里先整理几个常见题型。==）
...
//(ceil) ！向上取整，要更多猪猪。log10(buckets)/ log10(times)注意这个算出来是浮点数是正确的。注意不能让其除0
```
...
>```
>
>那么拓展，如果有多轮呢？那么只需要从二进制转换为多进制就好啦
...
如下图 用三进制来表示0~(n-1)的水的编号（个人理解：三进制是不是意味着猪一共喝两轮，以此类推，x进制有几位数意味着有几只猪），那么某个数用三进制表示，0表示第一轮喝了，1表示第二轮喝了，2表示没喝
...
如下图 不同的颜色代表不同的猪。哪几只猪死了，代表着唯一的毒药是哪个

![image-20250729234315562](assets/image-20250729234315562.png)

![image-20250410114641332](assets/image-20250410114641332.png)
...
>
>#### 问题核心
>- **目标**：在 N 枚硬币中找出唯一较重的一枚
>- **工具**：一架天平（每次称重有 3 种结果）
>- **关键策略**：利用天平的三种结果（左重、右重、平衡）将问题规模缩小到 1/3
>
>---
>
>### 三进制分组策略
...
>3. 平衡
>
>每次称重相当于获得 **1 个三进制位的信息**，k 次称重可区分 3ᵏ 种可能性，正好对应最多 3ᵏ 枚硬币。
>
>---
>
>### 关键总结
>1. **分组原则**：每次分成尽可能相等的 3 组
>2. **次数公式**：k = ⌈log₃N⌉
>3. **效率证明**：
>   - 最坏情况下每次缩小到 ≈N/3
>   - k 次后范围缩小到 1（3ᵏ ≥ N）
>4. **物理意义**：每个称重动作获得 1 个三进制位的信息量
>
>> 该策略是**信息论**的经典应用——通过最大化每次称重获得的信息量，达到最优检测效率。



### （3）砝码称重 :new:

>  你有一个天平，两边都可以放砝码。 你可以设计一些砝码，使得他们能够称量出1~n克的重量，请问最少需要多少砝码，
...
（等比数列推导，具体见D:\PGPostgraduate\githubNotePrepareForWork\PrepareForWorkNotes\八股文\C++\知识点\等比数列求和：幂次和公式详解.md）
...
答：取两次，返回11或00就重新取，直到取到01或10，因为这两个情况出现概率相等

给定函数F，返回0的概率是0.6，返回1的概率是0.4。我们需要设计一个新函数G，使得它返回0和1的概率均为0.5。可以利用拒绝采样的方法，即通过多次调用F来消除偏差。

具体步骤如下：

1. 连续调用F两次，得到两个结果a和b。
2. 如果a和b的组合是(0, 1)，则返回0。
3. 如果a和b的组合是(1, 0)，则返回1。
4. 其他情况（(0,0)或(1,1)）则拒绝并重新采样。
...
- 由于节点`i`的左孩子索引为`2i`，右孩子索引为`2i+1`，所以判断是左孩子只需要判断索引值是否为偶数即可。索引值为偶数表示是左孩子。**这个索引与线段树也是能对应上的。**
...
## 【2】游戏里什么时候会用到图论算法？能否举一些例子

接水管，扫雷，迷宫生成，消消乐这些都是，遮挡剔除——门剔除算法也会用到。
...
可以参考：https://github.com/hhlovesyy/CatDreamGame/blob/master/Assets/Programmer/Framework/Framework/Other/EventManager.cs#L20
