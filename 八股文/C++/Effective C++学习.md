...
```c++
// File 1:头文件
extern FileSystem tfs;  //extern 关键字的作用是 声明（而非定义）一个全局变量 tfs，表明该变量在其他编译单元（通常是另一个 .cpp 文件）中已经被定义。extern 声明通常放在头文件（.h）中，供多个源文件共享；实际定义放在某个 .cpp 文件中。
...
当然，这种做法对于多线程来说并不具有优势，最好还是在单线程启动阶段手动调用函数完成初始化。（==仅限before C++ 11==）
