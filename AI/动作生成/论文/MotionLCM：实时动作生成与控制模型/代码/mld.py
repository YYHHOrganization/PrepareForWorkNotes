...
class MLD(BaseModel):
    def __init__(self, cfg: DictConfig, datamodule: BaseDataModule) -> None:
        ...
        if self.is_controlnet:
            ...
            # [场景 尝试]
            # # 尝试新增场景编码器（以PointNet为例） -----------
            # self.scene_encoder = instantiate_from_config(cfg.model.scene_encoder) if cfg.model.get('scene_encoder') else None 
            # # 融合场景与轨迹特征
            # self.fusion_layer = nn.Linear(
            #     traj_encoder.output_dim + scene_encoder.output_dim, 
            #     controlnet.input_dim
            # )
            # # --------------------------------------------

            self.vaeloss = cfg.model.get('vaeloss', False)  # 是否启用潜在空间+运动空间联合监督 # 双空间监督标志
            self.vaeloss_type = cfg.model.get('vaeloss_type', 'sum')  # 双空间损失融合方式（求和或加权）
            self.cond_ratio = cfg.model.get('cond_ratio', 0.0)        # 条件损失权重
            self.rot_ratio = cfg.model.get('rot_ratio', 0.0)          # 关节旋转损失权重
            self.control_loss_func = cfg.model.get('control_loss_func', 'l2')  # 控制损失函数类型 # 损失函数类型
            ...
            self.use_3d = cfg.model.get('use_3d', False)       # 是否启用3D轨迹控制
            self.guess_mode = cfg.model.get('guess_mode', False)  # 是否启用ControlNet的"猜测模式"
            ...
            self.lcm_w_min_nax = cfg.model.get('lcm_w_min_nax') # 潜在一致性模型的权重范围
            self.lcm_num_ddim_timesteps = cfg.model.get('lcm_num_ddim_timesteps')  # 扩散步数
    ...
    def forward(self, batch: dict) -> tuple:
        ...
        joints = self.feats2joints(feats_rst.detach().cpu()) # 特征转关节坐标
    ...
    def _diffusion_reverse(
        ...
        for i, t in tqdm.tqdm(enumerate(timesteps)):
            ...
            # predict the noise residual # 去噪网络预测  预测噪声/潜在变量
    ...
    def train_diffusion_forward(self, batch: dict) -> dict:
        ...
        if self.is_controlnet and self.vaeloss:
            ...
            if self.cond_ratio != 0:
                ...
                # ----------------------------------------------------
                # 在cond_loss计算后添加：
                if self.collision_ratio > 0:
                    scene_voxel = batch['scene_voxel']  # 场景体素化表示
                    joints_proj = project_to_voxel(joints_rst)  # 关节投影
                    collision_loss = compute_collision(joints_proj, scene_voxel)
                    loss_dict['collision_loss'] = self.collision_ratio * collision_loss
                # ----------------------------------------------------
