...
def show_first_batch(loader, save_path=None):
    for batch in loader:
        show_images(batch[0], "Images in the first batch", save_path)
        break

def show_forward(ddpm, loader, device, save_path=None):
...
def generate_new_images(
    ...
    with torch.no_grad():
        ...
        for idx, t in enumerate(list(range(ddpm.n_steps))[::-1]):
            ...
            eta_theta = ddpm.backward(x, time_tensor) # 预测对应的噪声
...
def training_loop(
    ...
    for epoch in tqdm(range(n_epochs), desc=f"Training progress", colour="#00ff00"):
        ...
        for step, batch in enumerate(
            ...
            t = torch.randint(0, n_steps, (n,)).to(device)  # torch.randint(low, high, size)
...
def main():
    ...
    # action:
    # "store_true" - 有这个参数就是True，没有就是False
    ...
    transform = Compose([ToTensor(), Lambda(lambda x: (x - 0.5) * 2)]) # x = (x - 0.5) * 2,[0,1]->[-1,1]
    ...
    if visualizing_tmp:
        show_first_batch(loader, "first_batch.png")
        show_forward(ddpm, loader, device, "forward.png")
